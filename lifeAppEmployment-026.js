...
function submitButtonCheck() {
	...
	if (employment_info_satisfied &&
			($household_income.val().length > 5 && $household_income.val().length < 11) &&
			!income_sources_errors &&
			($living_expenses.val().length > 3 && $living_expenses.val().length < 11) &&
			...
			conditionals_for_other_insurance_satisfied)
	{
      document.getElementById("submit_button").disabled = false;
			...
			/*
			if (!employment_info_satisfied) {console.log("employment_info_satisfied = "+employment_info_satisfied);}
			if ($household_income.val().length <= 5 && $household_income.val().length >= 11) {console.log("$household_income.val().length = "+$household_income.val().length);}
			if (income_sources_errors) {console.log("income_sources_errors = "+income_sources_errors);}
			if ($living_expenses.val().length <= 3 && $living_expenses.val().length >= 11) {console.log("$living_expenses.val().length = "+$living_expenses.val().length);}
			if (isNaN(installment_debt) && installment_debt < 0 && installment_debt > 100) {console.log("installment_debt = "+installment_debt);}
			if ($net_worth.val() == "") {console.log("$net_worth.val() = "+$net_worth.val());}
			if (bankruptcy_selected == null) {console.log("bankruptcy_selected = "+bankruptcy_selected);}
			if (other_insurance_decline_selected == null) {console.log("other_insurance_decline_selected = "+other_insurance_decline_selected);}
			if (!conditionals_for_other_insurance_satisfied) {console.log("conditionals_for_other_insurance_satisfied = "+conditionals_for_other_insurance_satisfied);}
			*/
	...
	if (employment_status_selected === "Employed") {
		if ($employer.val().length > 0 &&
		    ...
		    $employment_info.css("border-color", no_error_color);
				...
				console.log("NEW EVALUATION OF WHETHER EMPLOYMENT INFO COMPLETE OR NOT");
				if ($employer.val().length == 0) {console.log("$employer.val().length = "+$employer.val().length);}
				if ($occupation.val().length == 0) {console.log("$occupation.val().length = "+$occupation.val().length);}
				if ($employer_address.val() == '') {console.log("$employer_address.val() = "+$employer_address.val());}
				if (formattedAddress == '') {console.log("formattedAddress = "+formattedAddress);}
				if ($employer_address_line1.val() == '') {console.log("$employer_address_line1.val() = "+$employer_address_line1.val());}
				if ($employer_city.val() == '') {console.log("$employer_city.val() = "+$employer_city.val());}
				if ($employer_state.val() == '') {console.log("$employer_state.val() = "+$employer_state.val());}
				if ($employer_zip.val() == '') {console.log("$employer_zip.val() = "+$employer_zip.val());}
				if ($years_employed.val().length == 0) {console.log("$years_employed.val().length = "+$years_employed.val().length);}
				if ($personal_income.val().length <= 5 || $personal_income.val().length >= 11) {console.log("$personal_income.val().length = "+$personal_income.val().length);}
				if (income_variability_selected == null) {console.log("income_variability_selected = "+income_variability_selected);}
	...
	(function($, window, document, undefined) {
      ...
      $installment_debt.mask('000%', { reverse: true });
			...
			$("input[name='EmploymentStatus']").click(function(e) {
				console.log("In the input[name='EmploymentStatus'].click() function");
				var employment_status_selected = $("input[name='EmploymentStatus']:checked").val();
				if (employment_status_selected == "Employed") {
...
/*
			$('form').on('change', 'input, select, textarea', function() {
				if (enabled) {
					console.log("In the $('form').on('change', 'input, select, textarea') function");
					submitButtonCheck();
				}
			});

			$('form').on('keyup', 'input, select, textarea', function() {
				console.log("In the $('form').on('keyup', 'input, select, textarea') function");
				if (enabled) {
					submitButtonCheck();
				}
			});
*/
			$employment_info.on('change focusout', function() {
				console.log("In the $employment_info.on('change focusout') function");
				var employment_status_selected = $("input[name='EmploymentStatus']:checked").val();
				if (employment_status_selected != null) {
					if (employment_status_selected == "Employed") {
						if (!employment_info_satisfied) {
							//$("#error-employment-info-required").hide(500);
							//$("#error-employment-info-all-fields-required").show(500);
							hideError($('#error-employment-info-required'));
							showError($('#error-employment-info-all-fields-required'));
					    $employment_info.css("border-color", error_color);
						} else if (employment_info_satisfied) {
							//$("#error-employment-info-required").hide(500);
							//$("#error-employment-info-all-fields-required").hide(500);
							hideError($('#error-employment-info-required'));
							hideError($('#error-employment-infos-all-fields-required'));
					    $employment_info.css("border-color", no_error_color);
						}
					} else if (employment_status_selected == "Unemployed") {
							//$("#error-employment-info-required").hide(500);
					...
					//$("#error-employment-info-required").show(500);
					//$("#error-employment-info-all-fields-required").hide(500);
					showError($('#error-employment-info-required'));
					hideError($('#error-employment-info-all-fields-required'));
			    $employment_info.css("border-color", error_color);
        ...
        submitButtonCheck();
			...
			$income_variability.on('change focusout', function() {
			  ...
			  if (selected != null) {
			    ...
			    //$("#error-income-variability-required").hide(500);
			    ...
			    //$("#error-income-variability-required").show(500);
			...
			function validateHouseholdIncome() {
			  const value = $household_income.val().trim();
			  if (value.length === 0) {
			...
			$income_sources.on('change', 'input', function () {
			...
			$income_sources.on('focusout', function () {
			  // Delay so we can check if the new focused element is still inside
			  setTimeout(() => {
      ...
      if (window.location.search) {
        ...
        if (employerAddress) {
          $('#floating-employer-address-label').addClass('float'); // Float the label
          $('#employer_address')
        ...
        if (yearsEmployed) {
          $('#floating-years-employed-label').addClass('float'); // Float the label
          $('#years_employed')
