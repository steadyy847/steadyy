...
//			$net_worth.val() != "" &&
			...
			conditionals_for_other_insurance_satisfied)
	{
      document.getElementById("submit_button").disabled = false;
		...
		console.log(
			"NEW EVALUATION FOR SUBMIT BUTTON"+"\n"+
			"employment_info_satisfied = "+employment_info_satisfied+"\n"+
			"household_income = "+$household_income.val().trim()+"\n"+
			"income_sources_errors = "+income_sources_errors+"\n"+
	    "income_tax_rate.length = "+$income_tax_rate.val().length+"\n"+
			"living_expenses.length = "+$living_expenses.val().length+"\n"+
			"installment_debt = "+installment_debt+"\n"+
			"emergency_fund_errors = "+emergency_fund_errors+"\n"+
			"liquid_assets.length = "+$liquid_assets.val().length+"\n"+
			"non_liquid_assets.length = "+$non_liquid_assets.val().length+"\n"+
			"short_term_debts.length = "+$short_term_debts.val().length+"\n"+
			"long_term_debts.length = "+$long_term_debts.val().length+"\n"+
			"anticipated_changes_errors = "+anticipated_changes_errors+"\n"+
			"bankruptcy_selected = "+bankruptcy_selected+"\n"+
			"other_insurance_decline_selected = "+other_insurance_decline_selected+"\n"+
			"conditionals_for_other_insurance_satisfied = "+conditionals_for_other_insurance_satisfied
		);
      ...
      function handlePercentageInput() {
        ...
        submitButtonCheck();
			...
			$emergency_fund.on('change', function() {
				console.log("In the emergency_fund.on('change') function")
			...
			$emergency_fund.on('focusout', function() {
				console.log("In the emergency_fund.on('focusout') function")
			  const emergency_fund_selected = $("input[name='EmergencyFund']:checked").val();
			  if (emergency_fund_selected != null) {
					// Emergency Fund radio button choice was made, so clear the error message
					hideError($('#error-emergency-fund-required'));
			    $emergency_fund.css("border-color", no_error_color);
					// But if 'No' was selected, but no reason was given as to why the user doesn't have an emergency fund, set the error flag to prevent submit button from being enabled
					if (emergency_fund_selected === "No" && $emergency_fund_explanation.val().trim().length === 0) {
						console.log("Setting the emergency_fund_errors = 'true' becase radio = 'No' and no explanation given");
						emergency_fund_errors = true;
					} else {
						console.log("Setting the emergency_fund_errors = 'false' becase either radio = 'no' or no explanation given");
						console.log("emergency_fund_selected = "+emergency_fund_selected);
						console.log("$emergency_fund_explanation.val().trim().length = "+$emergency_fund_explanation.val().trim().length);
						emergency_fund_errors = false;
					}
			  } else {
					// Emergency Fund radio button choice was not selected, so show the 'Field is required' error message
			...
			// Validate the Emergency Fund Explanation field either when the user enters new input, or moves focus away from it
			$emergency_fund_explanation.on('input focusout', function() {
			  const value = $emergency_fund_explanation.val().trim();
			  if (value.length === 0) {
					// if the emergency fund explanation field is blank, throw up an error message that this field is required 
          ...
          // if there is something selected in this field, clear the error message
			...
			*/
			...
			$anticipated_changes.on('change', 'input[type="checkbox"]', function () { // use 'change' for checkboxes
			  const id = this.id;
			  ...
			  if (id === 'no-changes' && this.checked) {
			    // If "no changes" is checked, uncheck the others
			    dependentFields.forEach(otherId => {
			      $('#' + otherId).prop('checked', false);
			      $('#' + otherId).prev('.w-checkbox-input').removeClass('w--redirected-checked');
			    });
			  }

			  if (dependentFields.includes(id) && this.checked) {
			    // If any individual change is checked, uncheck "no changes"
			    $('#no-changes').prop('checked', false);
			    $('#no-changes').prev('.w-checkbox-input').removeClass('w--redirected-checked');
			  }

			  validateAnticipatedChanges(); // Always revalidate after changes
			});
			...
			$anticipated_changes.on('focusout', function () {
			  ...
			  anticipatedFocusTimer = setTimeout(() => {
			    ...
			    if (!stillInside) {
			      validateAnticipatedChanges(); // Only validate once user has fully left
			    }
			  }, 10);
			});

			function validateAnticipatedChanges() {
			  const ids = [
			    'income-change',
			    'living-expenses-change',
			    'assets-change',
