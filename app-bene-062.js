...
function setupPlaceListener(beneNum) {
  setupFloatingLabelForAddress(beneNum);

  const autoEl = document.getElementById(`bene_${beneNum}_autocomplete`);
  if (!autoEl) return;

  autoEl.addEventListener('gmpx-placechange', () => {
    const place = autoEl.value;
    if (!place || !place.addressComponents) return;

    const components = place.addressComponents;

    let streetNumber = '', route = '', city = '', state = '', zip = '', addressLine2 = '';
    ...
    components.forEach(component => {
      ...
      if (component.types.includes('subpremise')) addressLine2 = component.longText;
    });

    const fullAddress = `${streetNumber} ${route}`.trim();
    const formattedAddress = place.formattedAddress;

    // Fill in your hidden fields
    $(`#bene_${beneNum}_formatted_address`).val(formattedAddress);
    $(`#bene_${beneNum}_street_number`).val(streetNumber);
    $(`#bene_${beneNum}_street`).val(route);
    $(`#bene_${beneNum}_address_line1`).val(fullAddress);
    $(`#bene_${beneNum}_address_line2`).val(addressLine2);
    $(`#bene_${beneNum}_city`).val(city);
    $(`#bene_${beneNum}_state`).val(state);
    $(`#bene_${beneNum}_zip`).val(zip);
...
function checkPrimaryBeneDetails() {
  ...
  if (primary_bene_selected == 'Yes') {
    for (let x = 1; x < primary_bene_ct + 1; x++) {
      ...
      console.log(
        "$('#bene_"+ x + "_first_name').val() = " + $('#bene_' + x + '_first_name').val() + "\n" +
        "$('#bene_" + x + "_last_name').val() = " + $('#bene_' + x + '_last_name').val() + "\n" +
        "$('#bene_" + x + "_dob').val() = " + $('#bene_' + x + '_dob').val() + "\n" +
        "$('#bene_" + x + "_phone_number').val().length = " + $('#bene_' + x + '_phone_number').val().length + "\n" +
        "$('#bene_" + x + "_address').val() = " + $('#bene_' + x + '_address').val() + "\n" +
        "$('#bene_" + x + "_formatted_address').val() = " + $('#bene_' + x + '_formatted_address').val() + "\n" +
        "$('#bene_" + x + "_relationship').val() = " + $('#bene_' + x + '_relationship').val() + "\n" +
      ...
      $('.phone-number').on('input.True', function () {
        if ($(this).val().length > 14) {
          $(this).val($(this).val().slice(0, 14));
			...
			// Loop over each element with the 'allocation-percentage' class
			$('.allocation-percentage').each(function () {
			  const $field = $(this);

			  // Apply initial mask to format the input as a percentage
			  $field.mask('000%', { reverse: true });

			  // Handle value changes as the user types
				$field.on('input', function () {
			    // Remove all non-digit characters
				  const raw = $field.val().replace(/\D/g, '');

				  if (raw === '') {
			      // If empty, clear and remove the mask so % doesn't show
				    $field.unmask();
				    $field.val('');
				  } else {
						let num = parseInt(raw, 10);

						// If 0% is entered display an error message
						if (num === 0) {
						  //$field.unmask();
						  //$field.val('');
		          $('#error-bene-1-allocation-required').hide(500);
		          $('#error-bene-1-allocation-valid-value').show(500);
						  return; // Exit early
						}

						// If >100% is entereed display an error message
						if (num > 100) {
							//num = 100;
		          $('#error-bene-1-allocation-required').hide(500);
		          $('#error-bene-1-allocation-valid-value').show(500);
						...
						if (num > 0 && num <= 100) {
		          $('#error-bene-1-allocation-valid-value').hide(500);
						}

				    const formatted = num + '%'; 	// Format as a percent
				    $field.val(formatted);

				    // Move the cursor before the '%' symbol
				    setTimeout(() => {
				      const input = $field.get(0);
				      const caretPos = formatted.length - 1; // just before '%'
				      input.setSelectionRange(caretPos, caretPos);
				    }, 0);
				  }
				});

			  // When refocusing the field, reapply the mask if % is not present
			  ...
			  // Handle backspace key so users can clear the field normally
...
// Modern Google Maps gmpx-place-autocomplete support for bene_1 to bene_5
function parsePlaceData(beneNum, place) {
  let streetNumber = '', route = '', city = '', state = '', zip = '', addressLine2 = '';
  ...
  s.setItem(`bene_${beneNum}_formatted_address`, formattedAddress);
  s.setItem(`bene_${beneNum}_street_number`, streetNumber);
  s.setItem(`bene_${beneNum}_street`, route);
  s.setItem(`bene_${beneNum}_address_line1`, fullAddress);
  s.setItem(`bene_${beneNum}_address_line2`, addressLine2);
  s.setItem(`bene_${beneNum}_city`, city);
  s.setItem(`bene_${beneNum}_state`, state);
  s.setItem(`bene_${beneNum}_zip`, zip);

  // Handle error display
  const $input = $(`#bene_${beneNum}_address`);
  const errorBase = `#error-bene-${beneNum}`;

  if (formattedAddress && fullAddress && city && state && zip) {
    $(`${errorBase}-address-not-valid`).hide(500);
    $(`${errorBase}-address-required`).hide(500);
    $input.css('border-color', '#C5C5C5');
  } else if (formattedAddress && (fullAddress || city || state || zip)) {
    $(`${errorBase}-address-required`).hide(500);
    $(`${errorBase}-address-not-valid`).show(500);
    $input.css('border-color', '#AC0036');
  }

  if (typeof submitButtonCheck === 'function') {
    submitButtonCheck();
  }
}
...
/*
customElements.whenDefined('gmpx-place-autocomplete').then(() => {
  console.log("Google Autocomplete component loaded");

  // Safe place to initialize your listeners
  initPlaceAutocomplete(); // Or your version like setupPlaceListener(i)
});
*/
...
customElements.whenDefined('gmpx-place-autocomplete')
  .then(() => {
    console.log("✅ gmpx-place-autocomplete web component is ready");

    // Initialize your address listeners AFTER component is ready
    for (let i = 1; i <= 5; i++) {
      setupPlaceListener(i);  // or initPlaceAutocomplete();
    }
  })
  .catch(err => {
    console.error("❌ Autocomplete component not loaded", err);
  });
