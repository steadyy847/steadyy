  ...
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      if (window.location.search) {
        ...
        // If all the required fields were complete when the page opens with query params in the URL, 
        // then reset the submitButton href to take user back to the saved applications page, instead of onto app page 2
        if (firstName != '' &&
            lastName != '' &&
            homeAddress != '' &&
            phoneNumber != '' &&
            ...
            // Update the link URL in #submitButton if URL included query parameters, which indicates user got here from the saved applications page
/*            var submitButtonLink = document.querySelector('#submit_button');
            if (submitButtonLink) {
                // Retrieve the current URL from the link block
                var currentUrl = submitButtonLink.href;
                console.log("Original submitButtonLink = "+currentUrl);

                // Reset the link URL to take user back to the saved applications page
                submitButtonLink.href = `${currentUrl}`;
                console.log("Updated submitButtonLink = "+submitButtonLink.href);
            }
*/


//            document.addEventListener('DOMContentLoaded', () => {
//              const formElement = document.querySelector('.application_form'); // Use your form's class or selector
//              const formElement = document.querySelector('form'); // Use your form's class or selector
              const formElement = document.querySelector('#application'); // Use your form's class or selector
              ...
              if (formElement) {
                ...
                hiddenInputs.forEach(input => {
                  ...
                  if (input.name === 'redirect' || input.name.includes('redirect')) {
                    console.log('Redirect URL found:', input.value); // This logs the redirect URL if it exists
                  }
                });
              } else {
...
//            });
      ...
      function submitButtonCheck() {
        var storage = window.localStorage,
          app_source_category = document.getElementById(
            'app_source_category'
          ).value;
        if (
          $app_first_name.val() != '' &&
            ...
            if (status === 'OK' && results[0]) {
              ...
              for (var i = 0; i < address_components.length; i++) {
                ...
                if (component.types.includes('street_number')) {
                  ...
                  document.getElementById('app_house_number').value =
                    house_number;
                  storage.setItem('app_house_number', house_number);
                }
                ...
                if (component.types.includes('subpremise')) {
                  ...
                  document.getElementById('app_address_line2').value =
                    address_line2;
                  storage.setItem('app_address_line2', address_line2);
                }
              ...
              if (house_number && street) {
                address_line1 = house_number + ' ' + street;
                document.getElementById('app_address_line1').value =
                  address_line1;
                storage.setItem('app_address_line1', address_line1);
                submitButtonCheck();
              }
