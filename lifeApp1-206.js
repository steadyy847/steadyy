...
// Utility: Normalize phone numbers (removes +1 and other formatting issues)
function normalizePhone(val) {
  let digits = val.replace(/\D/g, ''); // Remove all non-digits
...
$(document).ready(function () {
  const phoneInput = $('#app_phone_number');
  phoneInput.val(normalizePhone(phoneInput.val())); // Normalize any prefilled value
  ...
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      $app_phone_number.on('input.True', function () {
        ...
        // If it starts with a '1' and has 11 digits, assume it's a US country code
      ...
      if (!gpaInput) {
        ...
        autocomplete.addListener('place_changed', function () {
          // Clear out and re-initialize the hidden fields and local storage for for address components everytime the place is changed  
          addressInLicensedState = false;

          formattedAddress = '';
          $('#app_formatted_address').val('');
          storage.setItem('app_formatted_address', '');

          house_number = '';
          $('#app_house_number').val('');
          storage.setItem('app_house_number', '');

          street = '';
          $('#app_street').val('');
          storage.setItem('app_street', '');

          address_line1 = '';
          $('#app_address_line1').val('');
          storage.setItem('app_address_line1', '');

          address_line2 = '';
          $('#app_address_line2').val('');
          storage.setItem('app_address_line2', '');

          city = '';
          $('#app_city').val('');
          storage.setItem('app_city', '');

          state = '';
          $('#app_state').val('');
          storage.setItem('app_state', '');

          postalCode = '';
          $('#app_zip').val('');
          storage.setItem('app_zip', '');
          ...
          // Address validation & licensing logic
          if (
            formattedAddress != '' &&
            address_line1 != '' &&
            city != '' &&
            ...
            // Check if the state of the address entered is within the collection of states that Steadyy is licensed in & show error message if not
            const validStates = ['IL','CA','FL','MI','NJ','NY','PA','TX','OH',]; // Add VA when licensed
            if (validStates.includes(state)) {
              addressInLicensedState = true;
            ...
            // If there was a Google Maps API returned location, (as evidenced by having a value in formattedAddress, but not in
            // all of the required sub fields), but it isn't a complete address (e.g. just city or neighborhood), show message that
            // need the user to help getting a 'good' formatted address
            $('#zip-alert-state').hide(500);
            $('#error-address-required').hide(500);
      ...
      if (window.location.search) {
        ...
        if (phoneNumber) {
          ...
          $('#app_phone_number')
            .val(normalizePhone(decodeURIComponent(phoneNumber)))
            .trigger('input'); // Assign the value and trigger input event
        }
        if (emailAddress) {
