  ...
  geocoder.geocode({ address: addressString }, function (results, status) {
    if (status === 'OK' && results[0]) {
      ...
      if (address_line2) {
        ...
        reFormattedAddress = `${street_number} ${street}, ${address_line2}, ${city}, ${state} ${zip}`; // ZIP included, no 'USA'
      } else {
        reFormattedAddress = `${street_number} ${street}, ${city}, ${state} ${zip}`; // ZIP included, no 'USA'
      }
