    ...
    $(form).each(function () {
      ...
      $('.allocation-percentage').each(function () {
        ...
        $field.on('input', function () {
          // Extract only digits and cap at 3
          let raw = $field.val().replace(/\D/g, '').slice(0, 3);
          const num = parseInt(raw, 10);

          // Reset both errors on each input event
          $errorRequired.hide(500);
          $errorInvalid.hide(500);
          ...
          // Only re-format if necessary, and avoid recursion
          const formatted = num + '%';
          if ($field.val() !== formatted) {
            // Temporarily unbind to avoid recursive input
            $field.off('input');
            $field.val(formatted);
            // Rebind after a short delay
            setTimeout(() => {
              $field.on('input', arguments.callee);
            }, 0);
          }

          // Check if the value is valid; if so, hide any lingering errors
          if (num >= 1 && num <= 100) {
            $errorRequired.hide(500);
            $errorInvalid.hide(500);
          }
