...
var storage = window.localStorage,
    ...
    no_error_color = '', // simply resets default css behavior defined in webflow console, rather than hard coding it
		...
		$life_insurance_experience_CB = $('#experience-life-insurance-CB'),
		$fixed_annuities_experience_CB = $('experience-fixed-annuities-CB'),
		$variable_annuities_experience_CB = $('experience-variable-annuities-CB'),
		$stocks_experience_CB = $('experience-stocks-CB'),
		$bonds_experience_CB = $('experience-bonds-CB'),
		$other_experience_CB = $('experience-other-CB'),
		$no_experience_CB = $('experience-none-CB'),
...
const experienceErrorFlags = {
  life_insurance: false,
  fixed_annuities: false,
  variable_annuities: false,
  stocks: false,
  bonds: false,
  other: false
};
...
function hasAnyExperienceErrors() {
  return Object.values(experienceErrorFlags).some(flag => flag === true);
}

function submitButtonCheck() {
	if (!hasAnyExperienceErrors() &&
			!financial_products_experience_errors &&
      ...
      document.getElementById('submit_button').disabled = true;
          ...
          // if there is something selected in this field, clear the error message
			...
			$financial_products_experience.on('change', 'input[type="checkbox"]', function () {
			  ...
			  if (isNone && this.checked) {
				...
				setTimeout(() => {
				  ...
				  submitButtonCheck();
				}, 0); // Allow DOM state to fully update first
			...
			financialProducts.forEach(p => {
			  ...
			  $(p.$input).on('input focusout', function () {
			    const value = $(this).val().trim();

			    if (value.length === 0) {
			      showError($('#' + p.errorId));
			      $(this).css("border-color", error_color);
			      experienceErrorFlags[key] = true;
			    } else {
			      hideError($('#' + p.errorId));
			      $(this).css("border-color", no_error_color);
			      experienceErrorFlags[key] = false;
			    }
			    // Recheck checkbox/input sync
			    validateFinancialProductsExperience();
			    // Then run the button state logic
			    submitButtonCheck();
			  });
			});
			...
			function validateFinancialProductsExperience(triggerElement = null) {
			  const anyChecked = financialProducts.some(p => $('#' + p.cbId).is(':checked'));
			  const noneChecked = $('#experience-none-CB').is(':checked');

			  if (!anyChecked && !noneChecked) {
			  ...
			  for (let key in experienceErrorFlags) {
			    experienceErrorFlags[key] = false;
			  }

			  // Validate each selected experience input
			  financialProducts.forEach(p => {
			    const $checkbox = $('#' + p.cbId);
			    const $input = $('#' + p.inputId);
			    const $error = $('#' + p.errorId);
			    ...
			    if ($checkbox.is(':checked')) {
			      ...
			      if (val.length === 0) {
			        showError($error);
			        $input.css('border-color', error_color);
			        experienceErrorFlags[key] = true;
			      } else {
			        hideError($error);
			        $input.css('border-color', no_error_color);
			      ...
			      // Clear error state if box is unchecked
			      hideError($('#' + p.errorId));
			      $('#' + p.inputId).css('border-color', no_error_color);
			    }
			  });



				// Final evaluation of group-level error flag
				if (!anyChecked && !noneChecked) {
				  financial_products_experience_errors = true;
				} else {
				  // If any individual experience field is still in error, mark group error true
				  financial_products_experience_errors = Object.values(experienceErrorFlags).some(flag => flag === true);
				}
          ...
          $('#app_email_address')
            ...
            .trigger('input'); // Assign the value and trigger input event
				...
				if ('experience-life-insurance-CB' in parsed) {
				  ...
				  $life_insurance_experience_CB.prop('checked', isChecked);
				  if (isChecked) {
				    $life_insurance_experience_CB.prev('.w-checkbox-input').addClass('w--redirected-checked');
			      $('#conditional-years-experience-life-insurance').show(500);
				  } else {
				    $life_insurance_experience_CB.prev('.w-checkbox-input').removeClass('w--redirected-checked');
				  }
				}

				if ('experience-fixed-annuities-CB' in parsed) {
				  const isChecked = parseBoolean(parsed['experience-fixed-annuities-CB']);
				  $fixed_annuities_experience_CB.prop('checked', isChecked);
				  if (isChecked) {
				    $fixed_annuities_experience_CB.prev('.w-checkbox-input').addClass('w--redirected-checked');
			      $('#conditional-years-experience-fixed-annuities').show(500);
				  } else {
				    $fixed_annuities_experience_CB.prev('.w-checkbox-input').removeClass('w--redirected-checked');
				  }
				}

				if ('experience-variable-annuities-CB' in parsed) {
				  const isChecked = parseBoolean(parsed['experience-variable-annuities-CB']);
				  $variable_annuities_experience_CB.prop('checked', isChecked);
				  if (isChecked) {
				    $variable_annuities_experience_CB.prev('.w-checkbox-input').addClass('w--redirected-checked');
			      $('#conditional-years-experience-variable-annuities').show(500);
				  } else {
				    $variable_annuities_experience_CB.prev('.w-checkbox-input').removeClass('w--redirected-checked');
				  }
				}

				if ('experience-stocks-CB' in parsed) {
				  const isChecked = parseBoolean(parsed['experience-stocks-CB']);
				  $stocks_experience_CB.prop('checked', isChecked);
				  if (isChecked) {
				    $stocks_experience_CB.prev('.w-checkbox-input').addClass('w--redirected-checked');
			      $('#conditional-years-experience-stocks').show(500);
				  } else {
				    $stocks_experience_CB.prev('.w-checkbox-input').removeClass('w--redirected-checked');
				  }
				}

				if ('experience-bonds-CB' in parsed) {
				  const isChecked = parseBoolean(parsed['experience-bonds-CB']);
				  $bonds_experience_CB.prop('checked', isChecked);
				  if (isChecked) {
				    $bonds_experience_CB.prev('.w-checkbox-input').addClass('w--redirected-checked');
			      $('#conditional-years-experience-bonds').show(500);
				  } else {
				    $bonds_experience_CB.prev('.w-checkbox-input').removeClass('w--redirected-checked');
				  }
				}

				if ('experience-other-CB' in parsed) {
				  const isChecked = parseBoolean(parsed['experience-other-CB']);
				  $other_experience_CB.prop('checked', isChecked);
				  if (isChecked) {
				    $other_experience_CB.prev('.w-checkbox-input').addClass('w--redirected-checked');
			      $('#conditional-years-experience-other').show(500);
				  } else {
				    $other_experience_CB.prev('.w-checkbox-input').removeClass('w--redirected-checked');
				  }
				}
