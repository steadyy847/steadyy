...
/* STEP 4 - Set up listeners on each field for input events & validate all date entry fields */
	$return.on('change', 'input, select, textarea', function() {
		if (enabled) {
			console.log("In the onChange() function of the return div block object");
	...
	$('#modal-step').click(function(e) {
		var nextPage;
		console.log("INITIAL DECLARE STATEMENT | nextPage = "+nextPage);
...
//			$('#modal-alert-required').show(500);
      ...
      var start_over_CB = document.getElementById('start_over').checked,
          continue_previous_CB = document.getElementById('continue_previous').checked;
      ...
      if (start_over_CB) {
					...
					modalObject.hide(450);
					modalObject.css('opacity', 0);
			} else if (continue_previous_CB) {
					var commit_point = storage.getItem('commit_point');
console.log("RIGHT BEFORE SWITCH CLAUSE | commit_point = "+commit_point)
					switch (commit_point) {
						case "0-needs":
							let modalObject = $('.modal');
							modalObject.hide(450);
							modalObject.css('opacity', 0);
							nextPage = "START WITH 0-NEEDS";
			      	break;
			      case "1-basics":
...
//							nextPage = "/life-quote-referral";
					...
					if (nextPage !== "ERROR" && nextPage !== "START WITH 0-NEEDS") {
							console.log("AFTER SWITCH CLAUSE | nextPage = "+nextPage);
							alert("about to navigate to another page");
		 					location.href = nextPage;
					} else {
							console.log("Cannot tell what the last page was that user was on, during a WIP quote. nextPage = "+nextPage);
					}
			}
		}
	});
...
//		storage.getItem('children_num')
    ...
    if (page == "modal") {
			console.log("In the submitButtonCheck() function while on the modal window");

      var start_over_CB = document.getElementById('start_over').checked,
          continue_previous_CB = document.getElementById('continue_previous').checked;

			console.log("start_over_CB = "+start_over_CB+" | continue_previous_CB = "+continue_previous_CB);

      if (start_over_CB) {
					$("input[id='continue_previous']:checked").prop('checked', false);
					$('#continue_previous').prev('.w-checkbox-input').removeClass('w--redirected-checked');
		      document.querySelector("input[id='start_over']").checked = true;
					$('#start_over').prev('.w-checkbox-input').addClass('w--redirected-checked');
					document.getElementById("modal-step").disabled = false;
					$modal_next.toggleClass(className, true);
          modal_errors = false;
			} else if (continue_previous_CB) {
					$("input[id='start_over']:checked").prop('checked', false);
					$('#start_over').prev('.w-checkbox-input').removeClass('w--redirected-checked');
		      document.querySelector("input[id='continue_previous']").checked = true;
					$('#continue_previous').prev('.w-checkbox-input').addClass('w--redirected-checked');
					document.getElementById("modal-step").disabled = false;
					$modal_next.toggleClass(className, true);
          modal_errors = false;
			} else if (!start_over_CB && !continue_previous_CB) {
