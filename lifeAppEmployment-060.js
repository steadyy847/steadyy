...
////////////////////////////
			$anticipated_changes.on('change', 'input[type="checkbox"]', function () { // use 'change' for checkboxes
			  validateAnticipatedChanges();
			});

			$anticipated_changes.on('input', 'input[type="text"], textarea', function () { // use 'input' for text input field
			  validateAnticipatedChanges();
			});
			...
			$anticipated_changes.on('focusout', function () {
			  setTimeout(() => { // Delay so we can check if the new focused element is still inside
			    if (!$(document.activeElement).closest('#anticipated_changes').length) {
			      validateAnticipatedChanges();
			    }
			  }, 0);
			});
			...
			function validateAnticipatedChanges() {
			  ...
			  if (anyChecked) {
			    ...
			    anticipated_changes_errors = false;
					...
					const isNoChangesChecked = document.getElementById('no-changes').checked;
					if (isNoChangesChecked) {
						/*
						$("input[id='income-change']:checked").prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$("input[id='income-change']:checked").prop('checked', false);
						$("input[id='living-expenses-change']:checked").prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$("input[id='living-expenses-change']:checked").prop('checked', false);
						$("input[id='assets-change']:checked").prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$("input[id='assets-change']:checked").prop('checked', false);
						$("input[id='medical-expenses-change']:checked").prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$("input[id='medical-expenses-change']:checked").prop('checked', false);
						*/
						$(income_change).prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$(income_change).prop('checked', false);
						$(living_expenses_change).prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$(living_expenses_change).prop('checked', false);
						$(assets_change).prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$(assets_change).prop('checked', false);
						$(medical_expenses_change).prev('.w-checkbox-input').removeClass('w--redirected-checked');
						$(medical_expenses_change).prop('checked', false);
					}
