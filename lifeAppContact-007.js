...
function parseAddress(addressString, callback) {
  ...
  geocoder.geocode({ address: addressString }, function(results, status) {
    if (status === 'OK' && results[0]) {
      ...
      //storage.setItem('app_home_address', formattedAddress);
...
function submitButtonCheck() {
  ...
  console.log(
    ...
    "$app_phone_number.val().length = "+$app_phone_number.val().length +"\n"+
    "$app_source_category.val().length = "+$app_source_category.val().length
  );
...
$(function () {
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      $app_last_name.on('inpput focusout', function () {
      ...
      if (window.location.search) {
        ...
        if (homeAddress) {
          ...
          $('#app_home_address')
            ...
            .trigger('input');
          //storage.setItem('app_home_address', homeAddress);
          /*
          ...
          */
          ...
          window.deferAddressParsing = function () {
            parseAddress(decodeURIComponent(homeAddress), function () {
              submitButtonCheck();
            });
          };
          */
