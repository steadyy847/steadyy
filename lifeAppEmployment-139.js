...
function initAutocomplete() {
  ...
  // This defers parseAddress until Maps is ready.  So if a parseAddress call was deferred, run it now
  if (typeof window.deferAddressParsing === 'function') {
		console.log("Calling deferred parseAddress...");
    window.deferAddressParsing();
    window.deferAddressParsing = null; // Clean up
	} else {
	  console.warn("window.deferAddressParsing was not a function");
	}
...
function submitButtonCheck() {
	console.log("In the submitButtonCheck() function. About to call checkEmploymentInfo() function");
	checkEmploymentInfo();
	console.log("Back from call to checkEmploymentInfo() function");
...
function checkEmploymentInfo() {
	...
	console.log("In the checkEmploymentInfo() function");
	console.log(
      ...
      if (window.location.search) {
        ...
        if (EmploymentStatus) {
		        ...
		        if (employer_address) {
		          ...
		          // Defer the parseAddress call until Google Maps is ready
		          window.deferAddressParsing = function () {
							  const decoded = decodeURIComponent(employer_address);
							  console.log("In deferred call to parseAddress with address:", decoded);								
		            parseAddress(decodeURIComponent(employer_address), function () {
									console.log("Inside callback after parseAddress finishes");
		              submitButtonCheck();
		            });
		          };
							console.log("window.deferAddressParsing function assigned");
						}
        ...
        geocoder.geocode({ address: addressString }, function(results, status) {
          if (status === 'OK' && results[0]) {
            const place = results[0];
						...
						console.log("Just successfully called geocoder with status === 'OK' and place = "+place);
            formattedAddress = place.formatted_address;
						$employer_formatted_address.val(formattedAddress);
						console.log("$employer_formatted_address.val() = "+$employer_formatted_address.val());
            storage.setItem('employer_formatted_address', formattedAddress);
