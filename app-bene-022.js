...
$(document).on('keydown', function (e) {
  if (e.key === 'Tab') {
    // Delay just a moment to let the browser move focus
    setTimeout(function () {
      const active = document.activeElement;
      console.log('Tab landed on:', active);
      console.log('Tag:', active.tagName, '| ID:', active.id, '| Name:', active.name || '-', '| Class:', active.className);
    }, 0);
  }
});

$('#bene_1_first_name').on('focus', function () {
  console.log('FIRST NAME FOCUSED by JS');
});
				...
				switch (primary_bene_ct) {
					case 0:
						$('#additional_primary_bene_1').show(500, function () {
							if (document.activeElement.id !== 'bene_1_first_name') {
			...
			$primary_bene.on('change', function() {
				...
				if (primary_bene_selected != null) {
					if (primary_bene_selected == "Yes") {
						...
						// If the 'Yes' radio button was selected, place the focus onto the first field of the first beneficiary (e.g. First Name)
						if ($('#additional_primary_bene_1').is(':visible')) {
							if (document.activeElement.id !== 'bene_1_first_name') {
							  $('#additional_primary_bene_1').show(500, function () {
							    $('#bene_1_first_name').focus();
							  });
