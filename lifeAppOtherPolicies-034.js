...
var storage = window.localStorage,
  ...
  $past_replacements = $('#past-replacements'),
  $past_replacements_explanation = $('#past_replacements_explanation'),
...
function submitButtonCheck() {
  ...
  var other_insurance_decline_selected = $(
    "input[name='OtherLifeInsuranceDecline']:checked"
  ).val();

  if (other_insurance_decline_selected != null &&
    	conditionals_for_other_insurance_satisfied &&
    	past_replacements_errors === false)
	{
  ...
  if (other_insurance_selected == 'Yes') {
    for (let x = 1; x < other_policy_ct + 1; x++) {
      if ($('#other_carrier_' + x).val() != '' &&
	        $('#other_policy_coverage_amt_' + x).val().length > 4 &&
	        $('#other_policy_coverage_amt_' + x).val().length < 11 &&
	        $('#other_policy_year_issued_' + x).val() != '' &&
	        $('#other_policy_num_' + x).val() != '' &&
	        $('#other_policy_type_' + x).val() != '' &&
	        $('#other_policy_purchase_method_' + x).val() != '' &&
        ...
        switch (x) {
          case 1:
            conditionals_for_policy1_satisfied = false;
            break;
          case 2:
  ...
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      $('form').on(
        ...
        function () {
          // use 'change' for checkboxes, radio buttons, and drop-down select fields
          if (enabled) {
            submitButtonCheck();
          }
        }
      ...
      $('form').on('input', 'input[type="text"], textarea', function () {
        // use 'input' for text input field
        if (enabled) {
          submitButtonCheck();
        }
      });
      ...
      $("input[name='OtherLifeInsurance']").click(function (e) {
        ...
        if (other_insurance_selected == 'Yes') {
          ...
          for (let i = 1; i < 6; i++) {
            ...
            $('#other_policy_purchase_method_' + i).val('');
            $('#other_policy_purchase_method_' + i + '_label').removeClass('float');
      ...
      function removePolicy(i) {
        ...
        for (let x = i; x < other_policy_ct; x++) {
          ...
          $('#other_policy_purchase_method_' + x).val($('#other_policy_purchase_method_' + target_policy_to_move).val());
          ...
          if ($("input[name='Policy-" + target_policy_to_move + "-Replacement-Flag']:checked").val() == 'Yes'
            ...
            $("input[name='Policy-" + target_policy_to_move + "-Replacement-Flag']:checked").val() == 'No'
          ) {
          ...
          $('#other_policy_purchase_method_' + target_policy_to_move).val('');
      ...
      $past_replacements.on('change', function () {
        const past_replacements_selected = $("input[name='PastReplacements']:checked").val();

        if (past_replacements_selected != null) {
          // Past Replacements radio button choice was made, so clear the error message
          hideError($('#error-past-replacements-required'));
          $past_replacements.css('border-color', no_error_color);
          if (past_replacements_selected == 'No') {
            $('#past_replacements_details').hide(500);

            setTimeout(function () {
              $past_replacements_explanation.val('');
              hideError($('#error-past-replacements-explanation-required'));
              $past_replacements_explanation.css('border-color',no_error_color);
            }, 300);
            past_replacements_errors = false;
          } else if (past_replacements_selected == 'Yes') {

            $('#past_replacements_details').show(500);
            if ($past_replacements_explanation.val().trim().length === 0) {
              showError($('#error-past-replacements-explanation-required'));
              $past_replacements_explanation.css('border-color', error_color);
              past_replacements_errors = true;
            } else {
              hideError($('#error-past-replacements-explanation-required'));
              $past_replacements_explanation.css('border-color',no_error_color);
          ...
          // Past Replacements radio button choice was not selected, so show the 'Field is required' error message
          showError($('#error-past-replacements-required'));
          $past_replacements.css('border-color', error_color);
          past_replacements_errors = true;
        }
      ...
      $past_replacements.on('focusout', function () {
        const past_replacements_selected = $("input[name='PastReplacements']:checked").val();
				
        setTimeout(() => {
          // Check if focus has truly left the entire radio group
          if (!$(document.activeElement).closest('#past-replacements').length) {
            if (past_replacements_selected != null) {
              hideError($('#error-past-replacements-required'));
              $past_replacements.css('border-color', no_error_color);

              if (past_replacements_selected === 'Yes' &&
                	$past_replacements_explanation.val().trim().length === 0) 
							{
                past_replacements_errors = true;
              } else {
                past_replacements_errors = false;
      ...
      $past_replacements_explanation.on('focusout', function () {
        const value = $past_replacements_explanation.val().trim();
      ...
      $past_replacements_explanation.on('input', function () {
        const value = $past_replacements_explanation.val().trim();
        if (value.length > 0) {
          hideError($('#error-past-replacements-explanation-required'));
          $past_replacements_explanation.css('border-color', no_error_color);
          ...
          showError($('#error-past-replacements-explanation-required'));
          $past_replacements_explanation.css('border-color', error_color);
          past_replacements__errors = true;
        }
        submitButtonCheck();
      });
      ...
      if (window.location.search) {
        ...
        const pastReplacements = getQueryParameter('past_replacements');

				console.log("pastReplacements = "+pastReplacements);
          ...
          if (pair.includes('=')) {
            ...
            if (value) {
              ...
              if (match) {
				...
				if (pastReplacements) {
				  // Step 1: Parse the string into key-value pairs
				  const parsedParams = {};
				  const pairs = pastReplacements.split(';');

				  pairs.forEach(pair => {
				    const [key, ...rest] = pair.split('=');
				    parsedParams[key] = rest.join('='); // In case the value includes '='
				  });

				  // Step 2: Apply to form fields

				  // Handle the PastReplacements radio buttons
				  if (parsedParams.PastReplacements === 'Yes' || parsedParams.PastReplacements === 'No') {
				    const val = parsedParams.PastReplacements;

				    $("input[name='PastReplacements']").each(function () {
				      const $this = $(this);
				      const isMatch = $this.val() === val;
				      $this.prop('checked', isMatch);
				      $this.prev('.w-radio-input').toggleClass('w--redirected-checked', isMatch);
				    });

						if (parsedParams.PastReplacements === 'No') {
							past_replacements_errors = false;
						} else if (parsedParams.PastReplacements === 'Yes') {
						  // Handle the explanation input
						  if (parsedParams.past_replacements_explanation) {
						    $past_replacements_explanation.val(parsedParams.past_replacements_explanation);
...
/*        if (pastReplacements) {
          if (pastReplacements == 'Yes') {
            $("input[name='PastReplacements'][value='No']").prop('checked',false);
            $("input[name='PastReplacements'][value='No']").prev('.w-radio-input').removeClass('w--redirected-checked');
            $("input[name='PastReplacements'][value='Yes']").prop('checked',true);
            $("input[name='PastReplacements'][value='Yes']").prev('.w-radio-input').addClass('w--redirected-checked');
          }
          if (pastReplacements == 'No') {
            $("input[name='PastReplacements'][value='Yes']").prop('checked',false);
            $("input[name='PastReplacements'][value='Yes']").prev('.w-radio-input').removeClass('w--redirected-checked');
            $("input[name='PastReplacements'][value='No']").prop('checked',true);
            $("input[name='PastReplacements'][value='No']").prev('.w-radio-input').addClass('w--redirected-checked');
          }
        }
