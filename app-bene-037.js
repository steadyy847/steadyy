        ...
        if ($(this).val().length > 14) {
          $(this).val($(this).val().slice(0, 14));
			...
			$('.allocation-percentage').each(function () {
			  ...
			  // Clear mask if user deletes all digits
			  $field.on('input', function () {
			    const val = $field.val().replace('%', '').replace(/\D/g, '');

			    if (val === '') {
			      // Temporarily unmask so the % disappears
			      $field.unmask();
			      $field.val('');
			      return;
			    }
			  ...
			  // Reapply mask when the user re-focuses the field
			...
			$('.allocation-percentage').on('input', function () {
			  let rawVal = $(this).val().replace('%', '');
			  const digits = rawVal.replace(/\D/g, ''); // Remove any non-digits

				console.log('digits.length = '+digits.length);
			  // If no digits left, clear the field completely
			  if (digits.length === 0) {
			    $(this).val('');
			    return;
			  }
