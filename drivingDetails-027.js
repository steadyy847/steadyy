	...
	(function($, window, document, undefined) {
		...
		$(form).each(function() {
          ...
          $violations_last_5years_wrapper = $form.find('#violations-last-5years-wrapper'),
					...
					$violations_last_6mo = $form.find( 'input[id="violations_last_6mo"]' ),
					$violations_last_year = $form.find( 'input[id="violations_last_year"]' ),
					$violations_last_2years = $form.find( 'input[id="violations_last_2years"]' ),
					$violations_last_3years = $form.find( 'input[id="violations_last_3years"]' ),
					$violations_last_5years = $form.find( 'input[id="violations_last_5years"]' ),

/*					$violations_last_6mo = document.getElementById("violations_last_6mo"),
					$violations_last_year = document.getElementById("violations_last_year"),
					$violations_last_2years = document.getElementById("violations_last_2years"),
					$violations_last_3years = document.getElementById("violations_last_3years"),
					$violations_last_5years = document.getElementById("violations_last_5years"),
*/
...
/* STEP 4 - Set up listeners on each field for input events & validate all date entry fields */
			...
			$('form').on('change', 'input, select, textarea', function() {
				if (enabled) {
					console.log("In onChange() function | this.id = "+this.id);
					submitButtonCheck();
				}
			});
			...
			$('form').on('keyup', 'input, select, textarea', function() {
				if (enabled) {
					console.log("In onKeyup() function | this.id = "+this.id);
					submitButtonCheck();
				}
			});

			$violations_last_6mo.on('change', 'input, select, textarea', function() {
				console.log("Total Number of Violations = "+parseInt($violations_last_6mo.value) + parseInt($violations_last_year.value) + parseInt($violations_last_2years.value) + parseInt($violations_last_3years.value) + parseInt($violations_last_5years.value));
				if (moving_violations_selected == "Yes" && (parseInt($violations_last_6mo.value) + parseInt($violations_last_year.value) + parseInt($violations_last_2years.value) + parseInt($violations_last_3years.value) + parseInt($violations_last_5years.value)) > 0) {
					$(".no_violations_selected_error").hide(500);
				}
			});
/*
			$violations_last_year.on('change', 'input, select, textarea', function() {
				if (moving_violations_selected == "Yes" && (parseInt($violations_last_6mo) + parseInt($violations_last_year) + parseInt($violations_last_2years) + parseInt($violations_last_3years) + parseInt($violations_last_5years)) > 0) {
					$(".no_violations_selected_error").hide(500);
				}
			});

			$violations_last_2years.on('change', 'input, select, textarea', function() {
				if (moving_violations_selected == "Yes" && (parseInt($violations_last_6mo) + parseInt($violations_last_year) + parseInt($violations_last_2years) + parseInt($violations_last_3years) + parseInt($violations_last_5years)) > 0) {
					$(".no_violations_selected_error").hide(500);
				}
			});

			$violations_last_3years.on('change', 'input, select, textarea', function() {
				if (moving_violations_selected == "Yes" && (parseInt($violations_last_6mo) + parseInt($violations_last_year) + parseInt($violations_last_2years) + parseInt($violations_last_3years) + parseInt($violations_last_5years)) > 0) {
					$(".no_violations_selected_error").hide(500);
				}
			});

			$violations_last_5years.on('change', 'input, select, textarea', function() {
				if (moving_violations_selected == "Yes" && (parseInt($violations_last_6mo) + parseInt($violations_last_year) + parseInt($violations_last_2years) + parseInt($violations_last_3years) + parseInt($violations_last_5years)) > 0) {
					$(".no_violations_selected_error").hide(500);
				}
			});
*/
			...
			$moving_violations.on('focusout', function() {
					moving_violations_errors = chkDrivingDetailErrors('moving_violations', 'focusout', $moving_violations, $form, last_focus);
					last_focus = "moving_violations";
			});
			...
			$violations_last_5years_wrapper.on('focusout', function() {
					moving_violations_last_5years_errors = chkDrivingDetailErrors('violations_last_5years', 'focusout', $violations_last_5years_wrapper, $form, last_focus);
					last_focus = "violations_last_5years";
			});
          ...
          if (submit_errors) {
            ...
            if (!drunk_driving_errors && !drunk_driving_date_errors && !reckless_driving_errors && !reckless_driving_date_errors && !license_suspension_errors && !license_suspension_date_errors && !moving_violations_errors && !moving_violations_last_6mo_errors && !moving_violations_last_year_errors && !moving_violations_last_2years_errors && !moving_violations_last_3years_errors && !moving_violations_last_5years_errors) {
							...
							var moving_violations_selected = $("input[name='MovingViolations']:checked").val(),
							...
							if (moving_violations_selected == "Yes" && (parseInt($violations_last_6mo) + parseInt($violations_last_year) + parseInt($violations_last_2years) + parseInt($violations_last_3years) + parseInt($violations_last_5years)) == 0) {
								$(".no_violations_selected_error").show(500);
							} else {
								$(".no_violations_selected_error").hide(500);
	   					  $(".page_error").show(500);
							}
...
/*
console.log("drunk_driving_selected = "+drunk_driving_selected+"\n"+
"reckless_driving_selected = "+reckless_driving_selected+"\n"+
"license_suspension_selected = "+license_suspension_selected+"\n"+
"moving_violations_selected = "+moving_violations_selected+"\n"+
"conditionals_for_drunk_driving_satisfied = "+conditionals_for_drunk_driving_satisfied+"\n"+
"conditionals_for_reckless_driving_satisfied = "+conditionals_for_reckless_driving_satisfied+"\n"+
"conditionals_for_license_suspensions_satisfied = "+conditionals_for_license_suspensions_satisfied+"\n"+
"conditionals_for_moving_violations_satisfied = "+conditionals_for_moving_violations_satisfied);
*/
				if (drunk_driving_selected != undefined
         		...
         		document.getElementById("driving-more-step").disabled = false;
      			$driving_more_next.toggleClass(className, true);
      			submit_errors = false;
 					  $(".page_error").hide(500);
				} else {
         		document.getElementById("driving-more-step").disabled = true;
      			$driving_more_next.toggleClass(className, false);
      			submit_errors = true;
				}
...
//        if (last_focus == "violations_last_5years") {chkDrivingDetailErrors('violations_last_5years', 'focusout', object, $form, last_focus);}
    ...
    if (objectAffected == 'violations_last_5years') {
				...
				if ((parseInt($violations_last_6mo) + parseInt($violations_last_year) + parseInt($violations_last_2years) + parseInt($violations_last_3years) + parseInt($violations_last_5years)) == 0) {
					$(".no_violations_selected_error").show(500);
				} else {
					$(".no_violations_selected_error").hide(500);
				}
