		...
		(function($, window, document, undefined) {
			...
			$(form).each(function() {
				...
				function removePolicy(i) {
					console.log("other_policy_ct (before update) = "+other_policy_ct);
					console.log("remove policy "+i)

					// If user is trying to delete policy out of order (e.g. not the last one), then move values of higher count policy(ies) to the targeted one for deletion first
					for (let x = i; x < other_policy_ct; x++) {
						var target_policy_to_move = x+1;

						$('#other_carrier_'+i).val($('#other_carrier_'+target_policy_to_move).val());
						$('#other_policy_num_'+i).val($('#other_policy_num_'+target_policy_to_move).val());
						$('#other_policy_year_issued_'+i).val($('#other_policy_year_issued_'+target_policy_to_move).val());
						$('#other_policy_coverage_amt_'+i).val($('#other_policy_coverage_amt_'+target_policy_to_move).val());

					  if ($("input[name='policy-"+i+"-replacement-flag']:checked").val() == "Yes") {
						  $("input[name='policy-"+target_policy_to_move+"-replacement-flag'][value='Yes']").prop("checked", true);
							$("input[name='policy-"+target_policy_to_move+"-replacement-flag'][value='Yes']").prev('.w-radio-input').addClass('w--redirected-checked');
						...
						} else if ($("input[name='policy-"+i+"-replacement-flag']:checked").val() == "No") {
						  $("input[name='policy-"+target_policy_to_move+"-replacement-flag'][value='No']").prop("checked", true);
							$("input[name='policy-"+target_policy_to_move+"-replacement-flag'][value='No']").prev('.w-radio-input').addClass('w--redirected-checked');
				  ...
				  $("input[name='policy-"+i+"-replacement-flag'][value='No']").prop("checked", false);
					...
					$('#additional_policy_'+i).hide(500);
					other_policy_ct = i-1;
					console.log("other_policy_ct (after update) = "+other_policy_ct);
					switch (i) {
