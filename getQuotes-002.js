...
*/
function setDataAttribs(field) {
	var fieldArray = document.querySelectorAll(".w-dyn-item ."+field);
	fieldArray.forEach( function(elem) {
		var fieldText = elem.innerText || elem.innerContent;
		elem.closest(".mix").setAttribute("data-"+field, fieldText);
	});
}

setDataAttribs("list_carrier_code");

filter_string = '.'+company_code;

//var mixer = mixitup('.container', {
var mixer = mixitup('#container-20', {
multifilter: {enable: true},
pagination: {limit: 1},
load: {filter: filter_string},
callbacks: {
		onMixFail: function(state) {
				console.log("Error reloading 20 year container of carriers");
		}
}
});
