...
function validateTotalAllocation() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    const dob = $(`#bene_${i}_dob`).val().trim();
    ...
    // validate that the DOB is a valid date
    const isValidFormat = /^(0[1-9]|1[0-2])\/(0[1-9]|[12]\d|3[01])\/\d{4}$/.test(dob);
    ...
    console.log("!isLogicalDate = "+!isLogicalDate);
    console.log("isFutureDate = "+isFutureDate);

    // Validate that all required fields are present and allocation is within range
    if (!first || !last || !dob || !phone || !address || !relation || alloc <= 0 || alloc > 100 || !isValidFormat || !isLogicalDate || isFutureDate) {
