$(document).ready(function() {
  ...
  /* STEP 1 - Set up masks for the fields that require them */
	...
	function setPageValues() {
		...
		// Track the fact that this page was opened for Meta/Facebook event tracking purposes
		fbq('trackCustom', 'NeedsBuilder Initiated', {city: storage.getItem('quote_initiated_from_city')});
...
$('#childcare-input').keypress(function(event) {
    ...
    storage.setItem('coverage_recommendation_method', "Needs");
		...
		// Track the fact that this page was opened for Meta/Facebook event tracking purposes
		// Function to get a storage item and provide a default value if null
		...
		// Prepare event data
		const eventData = {
		    age: getStorageItem('age') + " (" + getStorageItem('dob') + ")",
		    income: getStorageItem('annual_income'),
		    coverage_recommendation: calculated_insurance_need,
		    city: getStorageItem('quote_initiated_from_city')
		};

		// Log event data to console for debugging
		console.log('Sending custom event:', eventData);

		// Send custom event to Facebook
		fbq('trackCustom', 'NeedsBuilderResultsViewed', eventData);
