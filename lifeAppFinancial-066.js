      ...
      document.getElementById('submit_button').disabled = true;
          ...
          // if there is something selected in this field, clear the error message
			...
			function validateOtherExperience() {
			  const otherChecked = $('#experience-other-CB').prop('checked');
			  const typeFilled = $other_type.val().trim().length > 0;
			  const yearsFilled = $years_of_other_experience.val().trim().length > 0;

			  if (otherChecked && typeFilled && yearsFilled) {
			    other_experience_errors = false;
			  } else if (otherChecked) {
			    other_experience_errors = true;
			  } else {
			    other_experience_errors = null; // not selected, don't block submit
			  }
