...
console.log("before initiatilizing autocomplete input and clearing all fields. address_line1 = "+address_line1);
...
console.log("in the placeChanged function, have house num and street, but before setting address_line1. address_line1 = "+address_line1);
            address_line1 = house_number + ' ' + street;
            storage.setItem('app_address_line1', address_line1);
          }
          if (address_line2) {
            //      document.getElementById("app_address_line2").value = address_line2;
...
console.log("at end of placeChanged function, checking that all subfields are full before clearing or clearing errors. address_line1 = "+address_line1);
        if (
          formattedAddress != '' &&
          address_line1 != '' &&
          city != '' &&
...
console.log("in the $app_home_address.on('input.True') function. address_line1 = "+address_line1);
        if (this.value.length == 0) {
          formattedAddress = '';
          house_number = '';
          street = '';
...
console.log("in focusout function of home address. address_line1 = "+address_line1);
      ...
      if (window.location.search) {
          ...
          if (homeAddress) {
              ...
              $('#app_formatted_address').val(decodeURIComponent(homeAddress)).trigger('input'); // Assign the value and trigger input event
          ...
          console.log("After setting all query params from URL into form fields is done. But before calling submitButtonCheck function to wrap it up:\n"+
                      "$app_home_address.val() = "+$app_home_address.val()+"\n"+
      ...
      function submitButtonCheck() {
        var storage = window.localStorage,
          app_source_category = document.getElementById('app_source_category').value;

        console.log("In the submitButtonCheck function:\n"+
                    "$app_first_name.val() = "+$app_first_name.val()+"\n"+
                    "$app_last_name.val() = "+$app_last_name.val()+"\n"+
                    "$app_home_address.val() = "+$app_home_address.val()+"\n"+
                    "formattedAddress = "+formattedAddress+"\n"+
                    "address_line1 = "+address_line1+"\n"+
                    "$app_address_line1.val() = "+$app_address_line1.val()+"\n"+              //test
                    "city = "+city+"\n"+
                    "$app_city.val() = "+$app_city.val()+"\n"+                                //test
                    "state = "+state+"\n"+
                    "$app_state.val() = "+$app_state.val()+"\n"+                              //test
                    "postalCode = "+postalCode+"\n"+
                    "$app_zip.val() = "+$app_zip.val()+"\n"+                                  //test
                    "$app_email_address.val() = "+$app_email_address.val()+"\n"+
                    "$app_phone_number.val() = "+$app_phone_number.val()+"\n"+
                    "$app_source_category.val() = "+$app_source_category.val()
        );
        ...
        storage.setItem('app_first_name', $app_first_name.val()); // Need for salutation on 'App Received' page
        storage.setItem('app_last_name', $app_last_name.val()); // Need for calendly pre-fill
        storage.setItem('app_phone_number', $app_phone_number.val()); // Need for calendly pre-fill
        storage.setItem('app_email_address', $app_email_address.val()); // Need for calendly pre-fill
      }
      ...
      function parseAddress(formattedAddress) {
        ...
        geocoder.geocode({ address: formattedAddress }, function(results, status) {
          if (status === "OK" && results[0]) {
            ...
            console.log("number of address_components = "+address_components.length);
            console.log("in the parseAddress() function after getting response from google but before evaluating results. address_line1 = "+address_line1);
            // Loop through address components to find the postal code, state, and city
            ...
            if (house_number && street) {
console.log("in the parseAddress() function, have house num and street, but before setting address_line1. address_line1 = "+address_line1);
              address_line1 = house_number + ' ' + street;
              document.getElementById('app_address_line1').value = address_line1;
              storage.setItem('app_address_line1', address_line1);
            }
