$(document).ready(function() {
  ...
  /* STEP 1 - Set up masks for the fields that require them */
	...
	$('#modal-step').click(function(e) {
		...
		console.log("modal_error = "+modal_errors);
		if (modal_errors) {
			e.preventDefault();
			$modal_next.toggleClass(className, false);
		} else {
			return_visit_selected = $("input[name='return_visit']:checked").val();
			console.log("return_visit_selected = "+return_visit_selected);

			if (return_visit_selected == "Restart") {
					console.log("quote_initiated_datetime (about to get zapped) = "+storage.getItem('quote_initiated_datetime'));
					storage.clear();
					console.log("previously_started_needs_builder (before resetting) = "+previously_started_needs_builder);
					previously_started_needs_builder = false;
					console.log("previously_started_needs_builder (after resetting) = "+previously_started_needs_builder);
					console.log("quote_initiated_datetime (after getting zapped) = "+storage.getItem('quote_initiated_datetime'));

					setPageValues();
					console.log("quote_initiated_datetime (after being reset) = "+storage.getItem('quote_initiated_datetime'));
