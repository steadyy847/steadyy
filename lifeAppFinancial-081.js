	...
	(function($, window, document, undefined) {
      ...
      document.getElementById('submit_button').disabled = true;
          ...
          // if there is something selected in this field, clear the error message
			...
			function validateFinancialProductsExperienceCheckboxes(forceValidateAll = false) {
			  let anyChecked = false;
			  let allValid = true;

			  $financial_products_experience.find('input[type="checkbox"]').each(function () {
			    const $checkbox = $(this);
			    const isChecked = $checkbox.is(':checked');
			    const checkboxId = $checkbox.attr('id');
			    const experienceInput = $('#' + checkboxId + '_experience');
			    const errorKey = checkboxId + '_experience_errors';

			    if (isChecked) {
			      anyChecked = true;
			      const value = experienceInput.val()?.trim();
			      if (value && !isNaN(value)) {
			        window[errorKey] = false;
			      } else {
			        window[errorKey] = true;
			        allValid = false;
			      }
			    } else if (forceValidateAll) {
			      window[errorKey] = null; // Explicitly mark as unvalidated
			    }
			  });

			  financial_products_experience_errors = !anyChecked || !allValid;
			}
			  ...
			  if (isNone && this.checked) {
			    ...
			    if (this.checked) {
			      $('#experience-none-CB').prop('checked', false).prev('.w-checkbox-input').removeClass('w--redirected-checked');
					...
					if (changedId === 'experience-other-CB' && this.checked) {
			          ...
			          case 'experience-bonds-CB':
			            ...
			            break;
								case 'experience-other-CB':
								  other_experience_errors = false; // Explicitly set it to false if "Other" is not checked
								  break;
								default:
      ...
      if (window.location.search) {
        ...
        if (emailAddress) {
          ...
          $('#other-type')
            .val(decodeURIComponent(otherType))
            .trigger('input'); // Assign the value and trigger input event
        }
        ...
        console.log("Done pre-filling fields from query parameters found in the URL");
				...
				// Force validation of the financial products experience checkboxes
				validateFinancialProductsExperienceCheckboxes(true);
				validateFinancialProductsExperience();
				validateOtherExperience();
