...
// create an object with the ranking of the values, with 'nt4' being the highest and 'smoke2' being the lowest
var riskRanking = {'nt1': 6, 'nt2': 5, 'nt3': 4, 'nt4': 3, 'smoke1': 2, 'smoke2': 1};
...
for (var i = 0; i < riskClasses.length; i++) {
  var riskClass = riskClasses[i];
	console.log("The " + riskClass.name + " variable has a value of: " + riskClass.value);
  
  // check if the risk class is not rated or not exist in the ranking
  if (riskClass === 'not rated' || !(riskClass in riskRanking)) {
    continue;
  }
  
  // check if the risk class is the smallest so far
  if (smallestValue === null || riskRanking[riskClass] < riskRanking[smallestValue]) {
    smallestValue = riskClass;
  }
}
