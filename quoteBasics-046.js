$(document).ready(function() {
  ...
  /*** STEP 8.1 - Listen for actiivity on the 'zip code' page ***/
    ...
    city = "Placeholder";
		...
		fetch('https://young-shelf-70816.herokuapp.com/usps/city-state?zip=' + encodeURIComponent(zip), {
		    method: 'GET',
		    headers: {
		...
		.catch(function(error) {
		    console.log('USPS ZIP lookup error:', error);
  ...
  /*** STEP 8.5 (FINAL STEP) - Listen for actiivity on the 'annual income' page ***/
	...
	function submitButtonCheck(page) {
  	...
  	if (page == "dob") {
		...
		// Calculate years to retirement age from current age, rounded to nearest term, so that we know what term duration to recommend to the customer
		var years_to_retirement = 65-Number(adj_age),
