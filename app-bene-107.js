    ...
    $(form).each(function () {
      ...
      $('.allocation-percentage').each(function () {
        ...
        $field.on('input', function () {
          ...
          setTimeout(() => {
            const val = $field.val();  // e.g. "99%"
            const raw = val.replace(/\D/g, '').slice(0, 3);
            const num = parseInt(raw, 10);
            console.log(`AFTER MASK → raw: ${raw}, num: ${num}`);
            ...
            if (isNaN(num) || num < 1 || num > 100) {
              ...
              // Valid value: hide both errors again just to be sure
              $errorRequired.hide(500);
              $errorInvalid.hide(500);
            }
      ...
      function validateAddress(formattedAddress) {
        return typeof formattedAddress === 'string' && formattedAddress.trim().length > 0;
      }
