...
var storage = window.localStorage,
  ...
  $bene_1_formatted_address = $('#bene_1_formatted_address'),
  ...
  $bene_2_formatted_address = $('#bene_2_formatted_address'),
  ...
  $bene_3_formatted_address = $('#bene_3_formatted_address'),
  ...
  $bene_4_formatted_address = $('#bene_4_formatted_address'),
  ...
  $bene_5_formatted_address = $('#bene_5_formatted_address'),
...
function initAutocomplete() {
  // Initialize the autocomplete inputs here
  console.log('Google Maps API initialized');

  for (let i = 1; i <= 5; i++) {
    const input = document.getElementById(`bene_${i}_address`);

    if (!input) {
      console.warn(`Address field bene_${i}_address not found.`);
      continue;
    }
    ...
    autocomplete.addListener('place_changed', function () {
      onPlaceChanged(input, i); // pass the specific input and index
    });

  }
}

// Global click detector for autocomplete dropdown items
...
function onPlaceChanged(inputElement, beneIndex) {
  //const place = autocomplete.getPlace();
  const place = new google.maps.places.Autocomplete(inputElement).getPlace();

  if (!place || !place.formatted_address) {
    inputElement.value = '';
    showAddressError(beneIndex);
    return;
  }

  console.log(`Place selected for bene_${beneIndex}_address:`, place);

  // Handle address parsing logic
  formattedAddress = place.formatted_address;
  storage.setItem(`bene_${beneIndex}1_formatted_address`, formattedAddress);
  $(`bene_${beneIndex}1_formatted_address`).val(formattedAddress);
...
function showAddressError(index) {
  $(`#error-bene-${index}-address-not-valid`).show(500);
  $(`#bene_${index}_address`).css('border-color', error_color);
}

function hideAddressError(index) {
  $(`#error-bene-${index}-address-not-valid`).hide(500);
  $(`#bene_${index}_address`).css('border-color', no_error_color);
}
