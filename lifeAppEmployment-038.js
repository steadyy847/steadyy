      ...
      $employer_address.on('input.True', function () {
        ...
        submitButtonCheck();
			...
			$income_sources.on('change', 'input, select, textarea', function () {
				console.log("Something changed about the income sources");
			  validateIncomeSources();
			});
			*/
			// For checkboxes — use 'change'
			$income_sources.on('change', 'input[type="checkbox"]', function () {
			  console.log("Something changed in a non-text field");
			  validateIncomeSources();
			});

			// For text input and textarea — use 'input' for live typing
			$income_sources.on('input', 'input[type="text"], textarea', function () {
			  console.log("User typed in text field");
			  validateIncomeSources();
			});
