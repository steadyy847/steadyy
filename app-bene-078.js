...
$(document).ready(function () {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    $dob.on('focusout', function () {
      ...
      let isLogicalDate = false, isFutureDate = false;

      if (isValidFormat) {
        const [mm, dd, yyyy] = value.split('/');
        const parsedDate = new Date(`${yyyy}-${mm}-${dd}`);
        isLogicalDate = parsedDate instanceof Date && !isNaN(parsedDate);
        isFutureDate = parsedDate > new Date();
      }
      ...
      if (value.length === 0) {
        ...
        $requiredError.show(500);
        $dob.css('border-color', error_color);
      } else if (!isValidFormat || !isLogicalDate || isFutureDate) {
...
$(function () {
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      // Loop through each allocation percentage field (should be one for each beneficiary)
      $('.allocation-percentage').each(function () {
        const $field = $(this);

        // Extract the beneficiary number from the field's ID (e.g., bene_3_allocation_percentage => 3)
        const id = $field.attr('id');
        const match = id.match(/^bene_(\d+)_allocation_percentage$/);
        const beneNum = match ? parseInt(match[1], 10) : null;

        if (!beneNum) return; // Skip this field if the ID format doesn't match

        // Cache error message elements for this specific beneficiary
        const $errorRequired = $(`#error-bene-${beneNum}-allocation-required`);
        const $errorInvalid = $(`#error-bene-${beneNum}-allocation-valid-value`);

        // Apply input masking to format as a 3-digit percentage (e.g., "85%")
        $field.mask('000%', { reverse: true });

        // Handle real-time user input to validate and format the value
        $field.on('input', function () {
          // Strip all non-digit characters to get the raw numeric input and limit to 3 characters
          let raw = $field.val().replace(/\D/g, '').slice(0, 3);

          if (!raw) {
            $field.unmask().val('');
            $errorRequired.show(500); /* Inserted - need to check that this is needed */
            $errorInvalid.hide(500); /* Inserted - need to check that this is needed */
            return;
          }
...
//        });
          ...
          /////////// OLD - START ///////////////
          if (raw === '') {
            // If the field is empty, unmask it, clear the value, and show the "required" error
            $field.unmask();
            $field.val('');
            $errorRequired.show(500);
            $errorInvalid.hide(500);
          } else {
            // Convert cleaned input to a number
            const num = parseInt(raw, 10);

            if (num === 0 || num > 100) {
              // Show "valid value" error if value is 0% or over 100%
              $errorRequired.hide(500);
              $errorInvalid.show(500);
            } else {
              // Input is between 1–100%, so hide all errors
              $errorRequired.hide(500);
              $errorInvalid.hide(500);
            }

            // Format the value as a string with a % sign (e.g., "42%")
            const formatted = num + '%';
            $field.val(formatted);

            // Reposition the cursor just before the % symbol
            setTimeout(() => {
              const input = $field.get(0);
              const caretPos = formatted.length - 1;
              input.setSelectionRange(caretPos, caretPos);
          ...
          */
          ...
          // Re-check if the form can be submitted
          submitButtonCheck();
        });
      ...
      function revalidateBeneFields(i) {
        ...
        // ADDRESS (validateAddress() depends on .formatted_address field)
        const formattedAddress = $(`#bene_${i}_formatted_address`).val();
        const $address = $(`#bene_${i}_address`);
        if (!formattedAddress) {
          $(`#error-bene-${i}-address-required`).show();
          $(`#error-bene-${i}-address-not-valid`).hide();
          $address.css('border-color', error_color);
          $(`#bene_${i}_address_label`).removeClass('float');
        } else if (!validateAddress(formattedAddress)) {
