...
$.get("https://ipinfo.io", function(response) {
		...
		$('#quote_initiated_datetime').val(getFormattedDate());
		storage.setItem('quote_initiated_datetime', getFormattedDate());
...
//	storage.clear();
	...
	console.log("In the initial page setting | commit_point = "+commit_point);
	console.log("In the initial page setting | navigation_to_0_needs = "+storage.getItem('navigation_to_0_needs'));

	if(commit_point !== null && commit_point !== "" && commit_point !== undefined && (storage.getItem('navigation_to_0_needs') !== "1-basics")) {
		let modalObject = $('.modal');
		modalObject.show(450);
		modalObject.css('opacity', 1);    
		modalObject.css('display', 'flex');
	}
	storage.setItem('navigation_to_0_needs', "");
...
console.log("In the setPageValues() fundtion. | commit_point = "+commit_point);
...
/* STEP 3 - Evaluate whether there was data in localStorage for the page, and re-initialize it with that data, if there was */
  ...
  if (storage_home_ownership !== null) {
	  $("input[name='home_ownership'][value='" + storage_home_ownership + "']").prop("checked", true);
		...
		if ($("input[name='home_ownership']").is(':checked') ) {
			...
			home_ownership_selected = $("input[name='home_ownership']:checked").val();
			console.log("BEFORE RESETTING: home_ownership_selected = "+home_ownership_selected);
			if (home_ownership_selected == "Yes" || home_ownership_selected == "No") {
			  $("input[name='home_ownership_'][value='" + home_ownership_selected + "']").prop("checked", false);
				$("input[name='home_ownership_'][value='" + home_ownership_selected + "']").prev('.w-radio-input').removeClass('w--redirected-checked');
			console.log("AFTER RESETTING: input[name='home_ownership_'][value='Yes'] = "+$("input[name='home_ownership']:checked").val());
			  $("input[name='home_ownership']").prop("checked", false);
			console.log("AFTER RESETTING: input[name='home_ownership_'] = "+$("input[name='home_ownership']:checked").val());
				...
				$home_next.toggleClass(className, false);
			console.log("AFTER RESETTING: home_ownership_selected = "+$("input[name='home_ownership']:checked").val());
			}
	}
  ...
  if (storage_children !== null) {
	...
	if (storage_children_num !== null) {
		...
		$('#child-age-input-1').val(null);
		$('#child-age-input-2').val(null);
		$('#child-age-input-3').val(null);
		$('#child-age-input-4').val(null);
		$('#child-age-input-5').val(null);
		$('#child-age-input-6').val(null);
		$('#child-age-input-7').val(null);
		$('#child-age-input-8').val(null);
		$('#child-age-input-9').val(null);
...
/* STEP 4 - Set up listeners on each field for input events & validate all date entry fields */
	...
	$('#modal-step').click(function(e) {
		...
		if (modal_errors) {
			e.preventDefault();
//			$('#modal-alert-required').show(500);
			document.getElementById("return-step").disabled = true;
			...
			if (return_visit_selected == "Continue") {
					var commit_point = storage.getItem('commit_point');
console.log("Right before switch statement | commit_point = "+commit_point);
					switch (commit_point) {
						case "0-needs":
			      	break;
			      case "1-basics":
	  					nextPage = "/life-quote-basics-slider";
			      	break;
			      case "2-citizenship":
	  					nextPage = "/life-quote-basics-citizenship";
			      	break;
			      case "3-health-profile":
	  					nextPage = "/life-quote-health-profile-slider";
			      	break;
			      case "4-tobacco":
...
alert("nextPage = "+nextPage);
					if (nextPage !== undefined) {
							...
							console.log("Cannot tell what the last page was that user was on, during a WIP quote.  commit_point = "+commit_point+" | nextPage = "+nextPage);
					}
			}
		}
	});
