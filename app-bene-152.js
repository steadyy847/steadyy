...
function checkPrimaryBeneDetails() {
  ...
  if (primary_bene_selected == 'Yes') {
    for (let x = 1; x < primary_bene_ct + 1; x++) {
      ...
      showOnlyErrorForField('bene-2-allocation', 'error-bene-2-allocation-valid-value');
    ...
    $(form).each(function () {
      ...
      function removePrimaryBene(i) {
        ...
        for (let x = i; x < primary_bene_ct; x++) {
          ...
          const fields = ['first_name', 'last_name', 'dob', 'phone_number', 'address', 'relationship', 'allocation'];
        ...
        pairs.forEach(pair => {
          if (pair.includes('=')) {
            ...
            if (value) {
              ...
              const match = key.match(/^bene_(\d+)_allocation$/);
