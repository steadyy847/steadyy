          ...
          // if there is something selected in this field, clear the error message
			...
			function validateLiquidAssets() {
			  ...
			  if (value.length === 0) {
				...
				var liquid_assets = parseInt($liquid_assets.val().trim().replace(/,/g, ''), 10) || 0,
				    ...
				    long_term_debts = parseInt($long_term_debts.val().trim().replace(/,/g, ''), 10) || 0,
						net_worth = liquid_assets + non_liquid_assets - short_term_debts - long_term_debts;
				
				//$('#calculated_net_worth').text('$' + net_worth.toLocaleString());
				let formattedNetWorth;

				if (net_worth < 0) {
				  // Format negative net worth: ($500,000)
				  formattedNetWorth = `($${Math.abs(net_worth).toLocaleString()})`;
				  $('#calculated_net_worth')
				    .text(formattedNetWorth)
				    .css('color', '#AC0036'); // sets color to same 'red' color of error messages
				} else {
				  // Format positive net worth: $500,000
				  formattedNetWorth = `$${net_worth.toLocaleString()}`;
				  $('#calculated_net_worth')
				    .text(formattedNetWorth)
				    .css('color', '#707070'); // sets to standard color
				}
			}
