...
var storage = window.localStorage,
    ...
    $years_to_keep_policy = $('#years_to_keep_policy'),
		...
		life_insurance_experience_errors = null,
		fixed_annuities_experience_errors = null,
		variable_annuities_experience_errors = null,
		stocks_experience_errors = null,
		bonds_experience_errors = null,
		other_experience_errors = null,
		financial_products_experience_errors = null;
...
function submitButtonCheck() {
	console.log("life_insurance_experience_errors = "+life_insurance_experience_errors);
	console.log("fixed_annuities_experience_errors = "+fixed_annuities_experience_errors);
	console.log("variable_annuities_experience_errors = "+variable_annuities_experience_errors);
	console.log("stocks_experience_errors = "+stocks_experience_errors);
	console.log("bonds_experience_errors = "+bonds_experience_errors);
	console.log("other_experience_errors = "+other_experience_errors);
	console.log("------------------------");
	console.log("financial_products_experience_errors = "+financial_products_experience_errors)
  ...
  const anyErrorsOrUntouched = experienceFlags.some(flag => flag !== false);

	if (!anyErrorsOrUntouched &&
...
$(document).ready(function () {
	Webflow.push(function(){
      ...
      if (window.location.search) {
        ...
        // If the URL has the saved_app query params and the user has already completed 6 steps, then reset the submitButton href
				// to take user back to the saved applications page, instead of onto app page 6 as if they were in normal flow
        if (savedApp && !isNaN(step) && step > 6) {
	...
	(function($, window, document, undefined) {
          ...
          // if there is something selected in this field, clear the error message
			...
			$financial_products_experience.on('change', 'input[type="checkbox"]', function () {
			  ...
			  if (isNone && this.checked) {
			    ...
			    // Set all individual experience flags to false
			    ...
			    // After toggling conditional fields, check what’s selected
			    let anyChecked = false;

			    financialProducts.forEach(p => {
			      const isChecked = $('#' + p.cbId).prop('checked');
			      ...
			      if (!isChecked) {
			        switch (p.cbId) {
			          ...
			          case 'experience-other-CB':
			            // Only set to false if both inputs for "Other" are valid
			            if ($other_type.val().trim().length > 0 && $years_of_other_experience.val().trim().length > 0) {
			              other_experience_errors = false;
			            }
			            break;
								default:
								  console.warn(`Unhandled checkbox ID: ${p.cbId}`);
								  break;									
			        }
					  } else {
					    // Set the corresponding flag to false only if the field is filled in
					    const value = p.$input.val().trim();
					    const isValid = value.length > 0 && !isNaN(Number(value));

					    switch (p.cbId) {
					      case 'experience-life-insurance-CB':
					        life_insurance_experience_errors = !isValid;
					        break;
					      case 'experience-fixed-annuities-CB':
					        fixed_annuities_experience_errors = !isValid;
					        break;
					      case 'experience-variable-annuities-CB':
					        variable_annuities_experience_errors = !isValid;
					        break;
					      case 'experience-stocks-CB':
					        stocks_experience_errors = !isValid;
					        break;
					      case 'experience-bonds-CB':
					        bonds_experience_errors = !isValid;
					        break;
					      case 'experience-other-CB':
					        // Already handled via the two separate handlers
					        break;
								default:
								  console.warn(`Unhandled checkbox ID: ${p.cbId}`);
								  break;									
			    ...
			    // If nothing is checked at all, keep financial_products_experience_errors = true
			    financial_products_experience_errors = !anyChecked;
					console.log("financial_products_experience_errors = "+financial_products_experience_errors);
			...
			/*
			$other_type.on('input focusout', function () {
			  const value = $other_type.val().trim();

				console.log("In the $other_type.on('input focusout') function");
				console.log("$other_type.val().length = "+$other_type.val().length);
			  if (value.length === 0) {
			    showError($('#error-other-type-required'));
			    $other_type.css("border-color", error_color);
			    other_experience_errors = true;
			  } else {
			    hideError($('#error-other-type-required'));
			    $other_type.css("border-color", no_error_color);
					console.log("$years_of_other_experience.val().length = "+$years_of_other_experience.val().length);
					if ($years_of_other_experience.val().trim().length === 0) {
				    other_experience_errors = true;
					} else {
				    other_experience_errors = false;
					}
			...
			$years_of_other_experience.on('input focusout', function () {
			  const value = $years_of_other_experience.val().trim();
				...
				console.log("In the $years_of_other_experience.on('input focusout') function");
				console.log("$years_of_other_experience.val().length = "+$years_of_other_experience.val().length);
			  if (value.length === 0) {
			    ...
			    $years_of_other_experience.css("border-color", no_error_color);
					console.log("$other_type.val().length = "+$other_type.val().length);
					if ($other_type.val().trim().length === 0) {
				    other_experience_errors = true;
					} else {
				    other_experience_errors = false;
					}
			...
			*/
			...
			$other_type.on('input focusout', validateOtherExperience);
			$years_of_other_experience.on('input focusout', validateOtherExperience);

			function validateOtherExperience() {
			  const typeVal = $other_type.val().trim();
			  const yearsVal = $years_of_other_experience.val().trim();

			  console.log("Validating Other Experience:");
			  console.log("Type length:", typeVal.length);
			  console.log("Years length:", yearsVal.length);
			  ...
			  other_experience_errors = (
			    $('#experience-other-CB').prop('checked') &&
			    (typeVal.length === 0 || yearsVal.length === 0)
			  );
			...
			function clearExperienceField({ conditionalId, inputId, errorId, $input, labelId }) {
			  $('#' + conditionalId).hide(500);
				if (conditionalId === 'conditional-years-experience-other') {
					$('#conditional-other-type').hide(500);
				  $('#other-type').val('');
				  hideError($('#error-other-type-required'));
				  $('#other-type').css('border-color', no_error_color);
				  $('#other_type_label').removeClass('float');
				}
			...
			function showExperienceField({ conditionalId }) {
			  $('#' + conditionalId).show(500);
				if (conditionalId === 'conditional-years-experience-other') {
					$('#conditional-other-type').show(500);
				}
			}
      ...
      if (window.location.search) {
        ...
        if (emailAddress) {
          ...
          $('#years_life_insurance_label').addClass('float'); // Float the label
          ...
          $('#years_fixed_annuities_label').addClass('float'); // Float the label
          ...
          $('#years_variable_annuities_label').addClass('float'); // Float the label
          ...
          $('#years_stocks_label').addClass('float'); // Float the label
          ...
          $('#years_bonds_label').addClass('float'); // Float the label
          $('#years-experience-bonds')
            ...
            .trigger('input'); // Assign the value and trigger input event
				...
				if (otherType) {
          $('#other_type_label').addClass('float'); // Float the label
          $('#other-type')
            .val(parseNumber(decodeURIComponent(otherType)))
            .trigger('input'); // Assign the value and trigger input event
        }

				if (yearsExperienceOther) {
          $('#years_other_label').addClass('float'); // Float the label
