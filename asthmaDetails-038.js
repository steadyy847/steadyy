...
/*        if (last_focus == "diagnosis_severity") {chkAsthmaDetailErrors('diagnosis_severity', 'focusout', $diagnosis_severity, $form, last_focus);}
        ...
        if (last_focus == "current_treatment") {chkAsthmaDetailErrors('current_treatment', 'focusout', $current_treatment, $form, last_focus);}
        if (last_focus == "treatment_types") {chkAsthmaDetailErrors('treatment_types', 'focusout', $treatment_types, $form, last_focus);}
        if (last_focus == "attacks_per_year") {chkAsthmaDetailErrors('attacks_per_year', 'focusout', $attacks_per_year_wrapper, $form, last_focus);}
        if (last_focus == "hospitalization") {chkAsthmaDetailErrors('hospitalization', 'focusout', $hospitalization, $form, last_focus);}
        if (last_focus == "hosp_past_year") {chkAsthmaDetailErrors('hosp_past_year', 'focusout', $hosp_past_year_wrapper, $form, last_focus);}
        if (last_focus == "hosp_total") {chkAsthmaDetailErrors('hosp_total', 'focusout', $hosp_total_wrapper, $form, last_focus);}
        if (last_focus == "hospitalization_date") {chkAsthmaDetailErrors('hospitalization_date', 'focusout', $hospitalization_date, $form, last_focus);}
*/
    } else if (objectAffected == 'diagnosis_severity') {
        $diagnosis_severity.css("border-color", "black");
        if (last_focus == "diagnosis_date") {chkAsthmaDetailErrors('diagnosis_date', 'focusout', object, object, $form, last_focus);}
        if (last_focus == "past_treatment") {chkAsthmaDetailErrors('past_treatment', 'focusout', $past_treatment, $form, last_focus);}
        if (last_focus == "current_treatment") {chkAsthmaDetailErrors('current_treatment', 'focusout', $current_treatment, $form, last_focus);}
        if (last_focus == "treatment_types") {chkAsthmaDetailErrors('treatment_types', 'focusout', $treatment_types, $form, last_focus);}
        if (last_focus == "attacks_per_year") {chkAsthmaDetailErrors('attacks_per_year', 'focusout', $attacks_per_year_wrapper, $form, last_focus);}
    ...
    if (objectAffected == 'diagnosis_date') {
        console.log("$diagnosis_month.val() = "+$diagnosis_month.val()+" | $diagnosis_year.val() = "+$diagnosis_year.val());
        console.log("$diagnosis_month.value = "+$diagnosis_month.value+" | $diagnosis_year.value = "+$diagnosis_year.value);
        if ($diagnosis_month.val() !== undefined && $diagnosis_year.val() !== undefined) {
  				if ($diagnosis_month.val().length > 0 && $diagnosis_year.val().length == 4) {
  					$("."+objectAffected+"_error").hide(500);
            object.css("border-color", no_error_color);
  					return false;
    		  } else {
  					$("."+objectAffected+"_error").show(500);
            $(".page_error").hide(500);
            object.css("border-color", error_color);
  					return true;
  				}
        } else {
  					$("."+objectAffected+"_error").show(500);
...
*/
    ...
    if (objectAffected == 'treatment_types') {
//	    	if () {
//					$("."+objectAffected+"_error").hide(500);
