...
function checkPrimaryBeneDetails() {
  ...
  if (primary_bene_selected == 'Yes') {
    for (let x = 1; x < primary_bene_ct + 1; x++) {
			/*
			const allocation = $('#bene_' + x + '_allocation_percentage').val().replace('%', '').trim();
			var allocation_valid = false;
			if (allocation === '' || isNaN(parseInt(allocation)) || parseInt(allocation) > 100) {
			  // Treat as invalid
