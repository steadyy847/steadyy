	...
	$(function() {
		...
		(function($, window, document, undefined) {
          ...
          storage = window.localStorage;
			...
			$(form).each(function() {
				var $form = $(this),
	     			$stroke_more_next = $form.find( '#stroke-more-step' ),
						currentPage = 9, // PAGE: stroke = 9
      			nextPage,
      			last_focus,
						enabled = true,
            submit_errors = true,
				...
				if (numericPartOfCommitPoint > 11) {
					$('#commit_point').val(commitPointItem);
				} else {
					storage.setItem('commit_point', "11-stroke");
					$('#commit_point').val("11-stroke");
				}
				storage.setItem('navigation_from', "11-stroke");
  			...
  			document.getElementById("stroke-more-step").disabled = true;
  			...
  			var storage_diag_month = storage.getItem('stroke_diag_month'),
  					storage_diag_year = storage.getItem('stroke_diag_year'),
						storage_episode_month = storage.getItem('stroke_episode_month'),
						storage_episode_year = storage.getItem('stroke_episode_year'),
						storage_other_details = storage.getItem('stroke_other_details');						
          ...
          document.getElementById("other_details").value = storage_other_details;
  			...
  			$diagnosis_date.on('focusin', function() {
  					diagnosis_date_errors = chkStrokeDetailErrors('diagnosis_date', 'focusin', $diagnosis_date, $form, last_focus);
  					last_focus = "diagnosis_date";
  			});

  			$episode_date.on('focusin', function() {
  					episode_date_errors = chkStrokeDetailErrors('episode_date', 'focusin', $episode_date, $form, last_focus);
  					last_focus = "episode_date";
  			});

  			$other_details_wrapper.on('focusin', function() {
  					other_details_errors = chkStrokeDetailErrors('other_details', 'focusin', $other_details_wrapper, $form, last_focus);
  					last_focus = "other_details";
  			});

			$other_details_wrapper.on('focusout', function() {
					other_details_errors = chkStrokeDetailErrors('other_details', 'focusout', $other_details_wrapper, $form, last_focus);
			});

  			$('#stroke-more-step').click(function(e) {
  				e.preventDefault();
          submitButtonCheck();
          ...
          if (submit_errors) {
            ...
            if (!diagnosis_date_errors && !episode_date_errors && !other_details_errors) {
        		...
        		document.getElementById("stroke-more-step").disabled = false;
  				  $(".page_error").hide(500);
  					location.href = nextPage;
          }
  			});
        
  			$('#stroke-more-prev').click(function(e) {
  				e.preventDefault();
  				window.history.back();
  			});
				...
				function submitButtonCheck() {
           		...
           		document.getElementById("stroke-more-step").disabled = false;
        			$stroke_more_next.toggleClass(className, true);
        			submit_errors = false;
   					  $(".page_error").hide(500);
					} else {
           		document.getElementById("stroke-more-step").disabled = true;
        			$stroke_more_next.toggleClass(className, false);
        			submit_errors = true;
          }

					storage.setItem('stroke_diag_month', $diagnosis_month.val());
					storage.setItem('stroke_diag_year', $diagnosis_year.val());
					storage.setItem('stroke_episode_month', $episode_month.val());
					storage.setItem('stroke_episode_year', $episode_year.val());
					storage.setItem('stroke_other_details', other_details);
...
function chkStrokeDetailErrors(objectAffected, focusState, object, $form, last_focus) {
...
//        if (last_focus == "diagnosis_date") {chkStrokeDetailErrors('diagnosis_date', 'focusout', object, $form, last_focus);}
        if (last_focus == "episode_date") {chkStrokeDetailErrors('episode_date', 'focusout', object, $form, last_focus);}
        if (last_focus == "other_details") {chkStrokeDetailErrors('other_details', 'focusout', object, $form, last_focus);}
