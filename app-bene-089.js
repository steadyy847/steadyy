    ...
    $phone.on('input', function () {
      ...
      */
      if (cleaned.length === 11 && cleaned.startsWith('1')) {
        ...
        if ($(this).val() !== newVal) {
          $(this).val(newVal);
          // do NOT call .trigger('input') here — let the user continue typing naturally
        }
      }
    ...
    $(form).each(function () {
      ...
      $('.allocation-percentage').each(function () {
        ...
        $field.on('input', function () {
          // Get only digits from the input and cap it to 3 digits
          let raw = $field.val().replace(/\D/g, '').slice(0, 3);

          // If the field is empty, clear mask, show 'required' error
          if (raw === '') {
            $field.unmask().val('');
            $errorRequired.show(500);
            $errorInvalid.hide(500);
          ...
          // If user enters 0, show 'invalid value' error
          if (num === 0) {
            $errorRequired.hide(500);
          ...
          // If user enters >100%, show 'invalid value' error
          if (num > 100) {
            $errorRequired.hide(500);
            $errorInvalid.show(500);
          } else {
            // Value is valid: hide both error messages
            $errorRequired.add($errorInvalid).hide(500);
          }

          // Format and display value with % sign
          const formatted = num + '%';
          ...
          // Reposition caret to just before the '%' symbol
          setTimeout(() => {
...
//            input.setSelectionRange(caretPos, caretPos);
            ...
            if (input && input.value.endsWith('%') && input.value.length <= 4) {
              const expectedCaretPos = input.value.length - 1;
              const currentCaretPos = input.selectionStart;
              console.log("inside the IF logic");
              console.log("expectedCaretPos = "+expectedCaretPos);
              console.log("currentCaretPos = "+currentCaretPos);

              // Only move caret if not already there to prevent infinite recursion
              if (currentCaretPos !== expectedCaretPos) {
                try {
                  input.setSelectionRange(expectedCaretPos, expectedCaretPos);
                } catch (e) {
                  console.warn('Caret error:', e);
                }
              }
            }
          }, 0);

          // Re-check whether the overall form is ready to submit
          submitButtonCheck();
        });
