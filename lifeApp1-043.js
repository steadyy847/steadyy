			...
			$(form).each(function() {
				var $form = $(this),
	    			...
	    			no_error_color = "#C5C5C5",
						...
						$source_category = $form.find('input[id="source_category"]');
	          ...
	          $phone_number.css("border-color", error_color);
				...
				$source_category.on('input.True', function() {
					console.log("---- In the onInput() function ----")
					console.log("$source_category.val() = "+$source_category.val());
					console.log("$source_category.val().length = "+$source_category.val().length);
					if (this.value.length == 0) {
	          ...
	          $source_category.css("border-color", no_error_color);
				...
				$source_category.on('focusout', function() {
					...
					console.log("$source_category.val() = "+$source_category.val());
					console.log("$source_category.val().length = "+$source_category.val().length);
					if ($source_category.val().length == 0) {
