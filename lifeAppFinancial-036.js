          ...
          // if there is something selected in this field, clear the error message
			...
			$financial_products_experience.on('focusout', function () {
			  financialProductsFocusTimer = setTimeout(() => {
			    ...
			    if (!stillInside) {
			      const anyChecked =
			        $('#experience-life-insurance-CB').prop('checked') ||
			        $('#experience-fixed-annuities-CB').prop('checked') ||
			        $('#experience-variable-annuities-CB').prop('checked') ||
			        $('#experience-stocks-CB').prop('checked') ||
			        $('#experience-bonds-CB').prop('checked') ||
			        $('#experience-other-CB').prop('checked') ||
			        $('#experience-none-CB').prop('checked');
			...
			$financial_products_experience.on('focusout', function () {
			  ...
			  financialProductsFocusTimer = setTimeout(() => {
			    const stillInside = $(document.activeElement).closest('#financial-products-experience').length > 0;
			    if (!stillInside) {
					  if ($('#experience-life-insurance-CB').prop('checked')) {
							if($years_of_life_insurance_experience.val().trim().length === 0) {
					  ...
					  if ($('#experience-fixed-annuities-CB').prop('checked')) {
							if($years_of_fixed_annuities_experience.val().trim().length === 0) {
					  ...
					  if ($('#experience-variable-annuities-CB').prop('checked')) {
							if($years_of_variable_annuities_experience.val().trim().length === 0) {
					  ...
					  if ($('#experience-stocks-CB').prop('checked')) {
							if($years_of_stocks_experience.val().trim().length === 0) {
					  ...
					  if ($('#experience-bonds-CB').prop('checked')) {
							if($years_of_bonds_experience.val().trim().length === 0) {
					  ...
					  if ($('#experience-other-CB').prop('checked')) {
						...
						if (life_insurance_experience_errors || fixed_annuities_experience_errors || variable_annuities_experience_errors || stocks_experience_errors || bonds_experience_errors || other_experience_errors) {
							console.log("In focusout. After cycling thru all Top 6, found at least one that was checked and with empty years text box");
							financial_products_experience_errors = true;
					    showError($('#error-financial-products-experience-required'));
					    $financial_products_experience.css('border-color', error_color);
						} else {
							console.log("In focusout. After cycling thru all Top 6, found No ERRORS of something checked and with empty years text box");
							financial_products_experience_errors = false;
					    hideError($('#error-financial-products-experience-required'));
					    $financial_products_experience.css('border-color', no_error_color);
							validateFinancialProductsExperience(this);
			...
			*/
			...
			function validateFinancialProductsExperience() {
			  ...
			  financialProducts.forEach(p => {
			    if ($('#' + p.cbId).prop('checked')) {
			      const val = p.$input.val().trim();
			      if (!val) {
			        showError($('#' + p.errorId));
			        p.$input.css('border-color', error_color);
			        hasError = true;
			      } else {
			        hideError($('#' + p.errorId));
			        p.$input.css('border-color', no_error_color);
			      }
			    }
			  });
			  ...
			  financial_products_experience_errors = hasError;
			}
