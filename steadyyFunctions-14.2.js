function calcPremium(thisObject) {
  const modal_factor = Number(
    $(thisObject).parents('.child-set_content').find('.modal_factor').text()
  );
  const policy_fee = Number(
    $(thisObject).parents('.child-set_content').find('.policy_fee').text()
  );
  var term = $(thisObject)
      .parents('.child-set_content')
      .find('.term')
      .text()
      .slice(4, 6),
    rate,
  ...
  if (!isNaN(parseInt(rate))) {
    var monthly_premium =
      (rate * modal_factor).toPrecision(4) * (coverage_amount / 1000) +
      policy_fee;
  } else {
...
function chkErrors(objectAffected, focusState, object, $form, last_focus) {
  ...
  if (focusState == 'focusin') {
    ...
    if (objectAffected == 'zip') {
      ...
      if (last_focus == 'tobacco') {
        chkErrors('tobacco', 'focusout', $tobacco, $form, last_focus);
      }
      if (last_focus == 'conditions') {
        chkErrors('conditions', 'focusout', $conditions, $form, last_focus);
      }
      if (last_focus == 'license') {
        chkErrors('license', 'focusout', $license, $form, last_focus);
      }
    } else if (objectAffected == 'weight') {
      $weight_div.css('border-color', 'black');
      if (last_focus == 'height') {
        chkErrors('height', 'focusout', $height_feet, $form, last_focus);
      }
      if (last_focus == 'tobacco') {
        chkErrors('tobacco', 'focusout', $tobacco, $form, last_focus);
      }
    ...
    if (objectAffected == 'dob') {
      if (
        $dob_month.val().length > 0 &&
        $dob_day.val().length > 0 &&
        $dob_year.val().length == 4
      ) {
        $('.' + objectAffected + '_error').hide(500);
    ...
    if (objectAffected == 'zip') {
      if (object.val().length == 5) {
        ...
        const validStates = [
          ...
          'TX',
          'OH',
        ]; // add "VA" after licensing is finished
    ...
    if (objectAffected == 'weight') {
      if (
        object.val().length >= 2 &&
        object.val().length <= 3 &&
        parseInt(object.val()) >= 75 &&
        parseInt(object.val()) <= 500
      ) {
        $('.' + objectAffected + '_error').hide(500);
        $('.' + objectAffected + '_error_validity').hide(500);
...
function dateDiff(startingDate, endingDate) {
  ...
  function parseDate(dateString) {
    ...
    return new Date(parsedDate.toISOString().substr(0, 10)); // Ensures YYYY-MM-DD format
  }

  // Parse startingDate
  let startDate;
  try {
    startDate = parseDate(startingDate);
  } catch (err) {
    console.error(err.message);
    return null; // Return null for invalid input
  }

  // Set or parse endingDate
  let endDate;
  try {
    endDate = endingDate ? parseDate(endingDate) : new Date();
  } catch (err) {
    console.error(err.message);
    return null; // Return null for invalid input
  }
  ...
  // Return the result as years, including fractional months and days
...
function checkDOB(document) {
  ...
  if (income_replacement_amount > 5000000) {
    ...
    storage.setItem(
      'income_replacement_amount_capped',
      income_replacement_amount
    );
  }
  storage.setItem('income_replacement_multiple', income_replacement_multiple);

  coverage_recommendation_method = storage.getItem(
    'coverage_recommendation_method'
  );
  if (coverage_recommendation_method !== 'Needs') {
    storage.setItem(
      'coverage_recommendation_method',
      'Multiple of Income (' + income_replacement_multiple + 'x)'
    );
    if (income_replacement_amount > 5000000) {
...
function getRiskClass(document) {
  ...
  var driving_drunk_dui = storage.getItem('driving_drunk_dui'),
    ...
    driving_violations_last_6mo = storage.getItem(
      'driving_violations_last_6mo'
    ),
    driving_violations_last_year = storage.getItem(
      'driving_violations_last_year'
    ),
    driving_violations_last_2years = storage.getItem(
      'driving_violations_last_2years'
    ),
    driving_violations_last_3years = storage.getItem(
      'driving_violations_last_3years'
    ),
    driving_violations_last_5years = storage.getItem(
      'driving_violations_last_5years'
    ),
    last_dui_date =
      storage.getItem('driving_last_dui_year') +
      '-' +
      storage.getItem('driving_last_dui_month') +
      '-01',
    last_reckless_date =
      storage.getItem('driving_last_reckless_year') +
      '-' +
      storage.getItem('driving_last_reckless_month') +
      '-01',
    last_suspension_date =
      storage.getItem('driving_last_suspension_year') +
      '-' +
      storage.getItem('driving_last_suspension_month') +
      '-01',
    years_since_dui = 99,
  ...
  if (driving_drunk_dui == 'Yes') {
    years_since_dui = dateDiff(last_dui_date, todayDate);
  }
  if (driving_reckless == 'Yes') {
    years_since_reckless = dateDiff(last_reckless_date, todayDate);
  }
  if (driving_license_suspension == 'Yes') {
    years_since_suspension = dateDiff(last_suspension_date, todayDate);
  }
  if (driving_moving_violations == 'Yes') {
    total_violations_in_past_3_years =
      Number(driving_violations_last_6mo) +
      Number(driving_violations_last_year) +
      Number(driving_violations_last_2years) +
      Number(driving_violations_last_3years);
  }

  var driving_risk_class = 'refer';
  switch (true) {
    case years_since_dui > 5 &&
      years_since_reckless > 5 &&
      years_since_suspension > 5 &&
      total_violations_in_past_3_years < 3:
      driving_risk_class = 'nt1';
      break;
    case years_since_dui > 3 &&
      years_since_reckless > 3 &&
      years_since_suspension > 3 &&
      total_violations_in_past_3_years < 4:
      driving_risk_class = 'nt3';
      break;
    case years_since_dui > 2 &&
      years_since_reckless > 2 &&
      years_since_suspension > 2 &&
      total_violations_in_past_3_years < 5:
  ...
  var tobacco = storage.getItem('tobacco'),
    ...
    last_cigarette_date =
      storage.getItem('tobacco_cigarette_last_smoke_year') +
      '-' +
      storage.getItem('tobacco_cigarette_last_smoke_month') +
      '-01',
    last_cigar_date =
      storage.getItem('tobacco_cigar_last_smoke_year') +
      '-' +
      storage.getItem('tobacco_cigar_last_smoke_month') +
      '-01',
    last_pipe_date =
      storage.getItem('tobacco_pipe_last_smoke_year') +
      '-' +
      storage.getItem('tobacco_pipe_last_smoke_month') +
      '-01',
    last_chew_date =
      storage.getItem('tobacco_chew_last_use_year') +
      '-' +
      storage.getItem('tobacco_chew_last_use_month') +
      '-01',
    last_patch_date =
      storage.getItem('tobacco_patch_last_use_year') +
      '-' +
      storage.getItem('tobacco_patch_last_use_month') +
      '-01';
  ...
  if (tobacco_cigarettes == 'Yes') {
    years_since_last_cigarette = dateDiff(last_cigarette_date, todayDate);
  }
  if (tobacco_cigars == 'Yes') {
    years_since_last_cigar = dateDiff(last_cigar_date, todayDate);
  }
  if (tobacco_pipe == 'Yes') {
    years_since_last_pipe = dateDiff(last_pipe_date, todayDate);
  }
  if (tobacco_chew == 'Yes') {
    years_since_last_chew = dateDiff(last_chew_date, todayDate);
  }
  if (tobacco_patch == 'Yes') {
    years_since_last_patch = dateDiff(last_patch_date, todayDate);
  }
  ...
  if (tobacco == 'Yes') {
    if (tobacco_cigars == 'Yes' && tobacco_cigars_per_month > 1) {
      years_since_last_tobacco_use = Math.min(
  ...
  var cancer = storage.getItem('cancer'),
    ...
    orig_cancer_diag =
      storage.getItem('cancer_diag_year') +
      '-' +
      storage.getItem('cancer_diag_month') +
      '-01',
  ...
  var heart_attack = storage.getItem('heart_attack'),
    last_heart_attack =
      storage.getItem('heart_attack_episode_year') +
      '-' +
      storage.getItem('heart_attack_episode_month') +
      '-01',
  ...
  var stroke = storage.getItem('stroke'),
    last_stroke =
      storage.getItem('stroke_episode_month') +
      '-' +
      storage.getItem('stroke_episode_month') +
      '-01',
  ...
  var depression = storage.getItem('depression'),
    ...
    orig_depression_diag =
      storage.getItem('depression_diag_year') +
      '-' +
      storage.getItem('depression_diag_month') +
      '-01',
    depression_current_treatment = storage.getItem(
      'depression_current_treatment'
    ),
  ...
  if (JSON.parse(depression)) {
    ...
    if (depression_current_treatment == 'Yes') {
      ...
      if (depression_current_treatment == 'No') {
        ...
        if (depression_past_treatment == 'Yes') {
          past_treatment_end_date =
            storage.getItem('depression_treatment_year') +
            '-' +
            storage.getItem('depression_treatment_month') +
            '-01';
          treatent_duration_in_years = dateDiff(
            orig_depression_diag,
            past_treatment_end_date
          );
        } else {
    ...
    if (
      depression_diagnosis != 'Severe' &&
      depression_hospitalization == 'No'
    ) {
      switch (true) {
        case depression_medications_num == 0:
          if (
            (depression_current_treatment =
              'No' && treatent_duration_in_years < 1)
          ) {
  ...
  var asthma = storage.getItem('asthma'),
    ...
    orig_asthma_diag =
      storage.getItem('asthma_diag_year') +
      '-' +
      storage.getItem('asthma_diag_month') +
      '-01',
  ...
  if (JSON.parse(asthma)) {
    ...
    switch (true) {
      case asthma_diagnosis == 'Mild' &&
        asthma_hospitalization == 'No' &&
        !inhaled_bronchodilators &&
        !inhaled_corticosteroids:
        asthma_risk_class = 'nt1';
        break;
      case asthma_hospitalization == 'No' &&
        asthma_treatment_current &&
        (inhaled_bronchodilators || inhaled_corticosteroids) &&
        tobacco == 'No':
  ...
  var sleep_apnea = storage.getItem('sleep_apnea'),
    ...
    orig_sleep_apnea_diag =
      storage.getItem('sleep_apnea_diag_year') +
      '-' +
      storage.getItem('sleep_apnea_diag_month') +
      '-01',
    ...
    sleep_apnea_rdi = storage.getItem(
      'sleep_apnea_respiratory_disturbance_index'
    ),
    sleep_apnea_oxygen_saturation = storage.getItem(
      'sleep_apnea_oxygen_saturation'
    ),
  ...
  if (JSON.parse(sleep_apnea)) {
    ...
    switch (true) {
      case sleep_apnea &&
        sleep_apnea_diagnosis == 'Mild' &&
        sleep_apnea_sleep_study == 'Yes' &&
        (sleep_apnea_apnea_index < 20 || sleep_apnea_rdi < 30) &&
        sleep_apnea_oxygen_saturation > 85:
        sleep_apnea_risk_class = 'nt2';
        break;
      case sleep_apnea &&
        sleep_apnea_diagnosis != 'Mild' &&
        sleep_apnea_cpap == 'Yes' &&
        years_since_sleep_apnea_diag > 0:
  ...
  for (var i = 0; i < riskClasses.length; i++) {
    ...
    if (riskClass.value !== 'not rated' && riskClass.value !== 'nt1') {
      ...
      factors_evaluated_for_risk_classes =
        factors_evaluated_for_risk_classes +
        riskClass.name +
        ' (' +
        riskClass.value +
    ...
    if (
      smallestValue === null ||
      riskRanking[riskClass.value] < riskRanking[smallestValue]
    ) {
  ...
  if (factors_evaluated_for_risk_classes == '') {
    ...
    factors_evaluated_for_risk_classes =
      factors_evaluated_for_risk_classes +
      'The final risk class (taking the lowest value) is: ' +
      smallestValue;
  }
