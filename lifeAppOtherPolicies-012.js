...
function submitButtonCheck() {
	...
	var past_replacements_selected = $("input[name='PastReplacements']:checked").val();

	if (other_insurance_decline_selected != null &&
			conditionals_for_other_insurance_satisfied &&
			past_replacements_selected)
	{
			...
			$past_replacements.on('focusout', function() {
			  setTimeout(() => {
			    if (!$(document.activeElement).closest('#past_replacements').length) {
			      const selected = $("input[name='PastReplacements']:checked").val();
			      if (!selected) {
			        showError($('#error-past_replacements-required'));
