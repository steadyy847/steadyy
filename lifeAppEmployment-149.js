...
$(function() {
	...
	(function($, window, document, undefined) {
      ...
      if (address_line2.length > 0) {
			...
			// Event for when the user interacts with the 'Are you Employed' question radio buttons
			$("input[name='EmploymentStatus']").click(function(e) {
				var employment_status_selected = $("input[name='EmploymentStatus']:checked").val();

				// If the user clicks the radio button that they are employed, show the hidden sub-fields asking about the employer and income
				if (employment_status_selected == "Employed") {
					$(".employment-details").show(500);
				// But when the user clicks the radio button that they are not employed, clear out all of the sub-fields about the employer and income
				} else if (employment_status_selected == "Unemployed") {
					$('#employer').val("");
      ...
      function handlePercentageInput() {
        ...
        submitButtonCheck();
			...
			$emergency_fund_explanation.on('input', function () {
				console.log("In the $emergency_fund_explanation.on('input') function");				
			  const value = $emergency_fund_explanation.val().trim();

				console.log("$emergency_fund_explanation.val() = "+value);
				console.log("$emergency_fund_explanation.val().length = "+value.length);				
			  if (value.length > 0) {
			    hideError($('#error-emergency-fund-explanation-required'));
			    $emergency_fund_explanation.css("border-color", no_error_color);
			    emergency_fund_errors = false;
        ...
        if (other_income_detail) {
          $('#other_income_detail')
            ...
            .trigger('input'); // Assign the value and trigger input event
				...
				// After all Income Sources checkboxes have been set, validate all fields in this section
				validateIncomeSources();
