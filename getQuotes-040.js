...
*/
	...
	*/
	...
	var [birthyear, birthmonth, birthday] = dob.split('-').map((part, index) => 
		...
		if (response.Compulife_ComparisonResults === undefined) {
			...
			termResult.forEach(result2 => {
				...
				if (Number(term) == recommended_term) {
						console.log("recommended_term = term. \n"+
												"Recommended Term = "+recommended_term+"\n"+
												"Recommended Term Carrier = "+company_code+"\n"+
												"Recommended Term Premium = "+premium_formatted
												);
						$('#recommended_term_carrier').val(company_code);
						$('#recommended_term_premium').val(premium_formatted);
				}
				if (Number(term) == 10) {
						console.log("Minimum Term = "+term+"\n"+
												"Minimum Term Carrier = "+company_code+"\n"+
												"Minimum Term Premium = "+premium_formatted
												);
