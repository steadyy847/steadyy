...
//				$net_worth.mask("#,##0", {reverse: true}); // Changed to a selectable drop-down
				...
				function removePolicy(i) {
					...
					for (let x = i; x < other_policy_ct; x++) {
						...
						console.log("In removePolicy() - Policy "+target_policy_to_move+" Replacement Flag = "+$("input[name='Policy "+target_policy_to_move+" Replacement Flag']:checked").val());
				...
				function checkAdditionalPolicyDetails() {
					...
					if (other_insurance_selected == "Yes") {
							for (let x = 1; x < other_policy_ct+1; x++) {
								console.log("In checkAdditionalPolicyDetails() - Policy "+x+" Replacement Flag = "+$("input[name='Policy "+x+" Replacement Flag']:checked").val());
