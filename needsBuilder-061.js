...
/* STEP 3 - Evaluate whether there was data in localStorage for the page, and re-initialize it with that data, if there was */
  if (storage_needs_help !== null) {
	  $("input[name='NeedsHelp'][value='" + storage_needs_help + "']").prop("checked", true);
		$("input[name='NeedsHelp'][value='" + storage_needs_help + "']").prev('.w-radio-input').addClass('w--redirected-checked');
  ...
  if (storage_home_ownership !== null) {
	  $("input[name='HomeOwnership'][value='" + storage_home_ownership + "']").prop("checked", true);
		$("input[name='HomeOwnership'][value='" + storage_home_ownership + "']").prev('.w-radio-input').addClass('w--redirected-checked');
...
//				document.getElementById("college-savings-step").disabled = true;
    ...
    storage.setItem('total_insurance_need', Math.ceil((total_insurance_need) / 100000) * 100000);
		...
		console.log("coverage_rec_method = "+coverage_rec_method+"\n"+
								"sliced = "+coverage_rec_method.slice(15,17));
