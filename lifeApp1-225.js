  ...
  if (
    ...
    for (i = 0; i < storage.length; i++) {
      if (
        ...
        storage.key(i) != 'topicsLastReferenceTime' &&
        storage.key(i) != 'ms_session_id'
      ) {
...
function onPlaceChanged() {
  ...
  // Set focus back to the home_address field but delay resetting focus slightly so we don't block normal tab behavior
  setTimeout(function() {
    // Only reset focus if it is NOT already in an input or textarea
    if (
  ...
  }, 100); // ← slightly longer timeout (~100ms) is safer for mouse clicks
}
