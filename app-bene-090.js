    ...
    $(form).each(function () {
      ...
      $('.allocation-percentage').each(function () {
        ...
        $field.on('input', function () {
          ...
          // Reposition caret to just before the '%' symbol, only for valid 1–100%
          setTimeout(() => {
            const input = $field.get(0);
            if (!input || !input.value.endsWith('%')) return;

            const raw = input.value.replace(/\D/g, '');
            const num = parseInt(raw, 10);

            if (isNaN(num) || num < 1 || num > 999) return; // Don't reposition caret for invalid values
