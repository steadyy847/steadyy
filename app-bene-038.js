        ...
        if ($(this).val().length > 14) {
          $(this).val($(this).val().slice(0, 14));
			...
			$('.allocation-percentage').each(function () {
			  const $field = $(this); // Store reference to this specific field

			  // Apply the input mask to allow 0-100% formatted as '##0%'
			  $field.mask('000%', { reverse: true });

			  // Handle changes as the user types into the field
			  $field.on('input', function () {
			    // Strip out all non-digit characters (removes %, etc.)
			    const raw = $field.val().replace(/\D/g, '');

			    // If the field is now empty (user deleted everything), unmask and clear it completely
			    if (raw === '') {
			      $field.unmask();       // Remove the % mask
			      $field.val('');        // Clear the input field
			    } else {
			      // Parse the numeric value and clamp to 100 if too high
			      let num = parseInt(raw, 10);
			      if (num > 100) num = 100;

			      // Manually format the value as a percentage (adds % symbol)
			      $field.val(num + '%');
			    }
			  });

			  // When the user focuses back into the field, reapply the mask if it's missing
			  $field.on('focus', function () {
			    if (!$field.val().includes('%')) {
			      // Only reapply the mask if the % isn't already present
			      $field.mask('000%', { reverse: true });
			    }
			  });
			});
