...
function submitButtonCheck() {
	...
	var income_variability_selected = $("input[name='IncomeVariability']:checked").val(),
...
$(function() {
	...
	(function($, window, document, undefined) {
      ...
      if (address_line2.length > 0) {
			...
			$employer.on('input focusout', validateEmployer);
			...
			$occupation.on('input focusout', validateOccupation);

			function validateYearsEmployed() {
          ...
          // if there is something selected in this field, clear the error message
			...
			$years_employed.on('change focusout', validateYearsEmployed);
      ...
      $employer_address.on('input.True', function () {
        ...
        submitButtonCheck();
			...
			function validatePersonalIncome() {
			  const value = $personal_income.val().trim();
			  if (value.length === 0) {
			...
			$personal_income.on('input focusout', validatePersonalIncome);

			function validateIncomeVariability() {
			...
			$income_variability.on('change focusout', validateIncomeVariability);
			...
			$household_income.on('input focusout', validateHouseholdIncome);
			...
			function validateIncomeSources() {
			  ...
			  if (anyChecked) {
					// if there is at least one checkbox selected, clear the error message
			    $('#error-income-sources-required').hide(500);
			    $income_sources.css('border-color', no_error_color);
			    income_sources_errors = false;
			  } else {
					// if none of the checkboxes are selected, show the error message that user must select at least one source for their income
			...
			// Validate the Income Sources checkboxes either when the user changes checkbox value, or moves focus away from it
			$income_sources.on('change focusout', 'input, select, textarea', validateIncomeSources);

			/*
			$income_sources.on('change', 'input, select, textarea', function() {
				if (enabled) {
					var wages_CB = document.getElementById('wages-income').checked,
							investments_CB = document.getElementById('investments-income').checked,
							rentals_CB = document.getElementById('rental-income').checked,
							social_security_CB = document.getElementById('social-security-income').checked,
							retirement_CB = document.getElementById('retirement-income').checked,
							other_CB = document.getElementById('other-income').checked;
					
					if (wages_CB || investments_CB || rentals_CB || social_security_CB || retirement_CB || other_CB) {
							$('#error-income-sources-required').hide(500);
					    $income_sources.css('border-color', no_error_color);
							income_sources_errors = false;
					} else {
