$(document).ready(function() {
	...
	parseField(basics_age);
	parseField(basics_location);
	parseField(health_profile_body_type);
  ...
  parseField(health_details_driving_risk);
	...
	alert("About to populate local storage with single attribute fields");
	console.log('email = '+email);
	storage.setItem('email', email);
	console.log('gender = '+basics_gender);
	storage.setItem('gender', basics_gender);
	console.log('tobacco = '+health_profile_smoker);
	storage.setItem('tobacco', health_profile_smoker);
	console.log('risk_class = '+risk_rating);
	storage.setItem('risk_class', risk_rating);
	console.log('annual_income = '+basics_income);
	storage.setItem('annual_income', basics_income);
	console.log('coverage_recommendation = '+coverage_recommendation);
	storage.setItem('coverage_recommendation', coverage_recommendation);
	console.log('recommended_term = '+recommended_term);
	storage.setItem('recommended_term', recommended_term);
	console.log('needs_help = '+needs_help);
	storage.setItem('needs_help', needs_help);
	console.log('license = '+health_profile_driving_risk);
...
// Enable the Login Modal when the Login is clicked on Nav Bar
...
var storage = window.localStorage,
		gender = storage.getItem('gender').slice(0,1).toLowerCase(),
