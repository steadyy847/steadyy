...
var storage = window.localStorage,
  ...
  reFormattedAddress,
  $app_street_number = $('#app_street_number'),
...
function parseAddress(addressString, callback) {
  ...
  geocoder.geocode({ address: addressString }, function (results, status) {
    if (status === 'OK' && results[0]) {
      ...
      if (street_number) {
        $('#app_street_number').val(street_number);
      }
      ...
      if (address_line2) {
        ...
        reFormattedAddress = `${street_number} ${route}, ${address_line2}, ${city}, ${state} ${zip}`; // ZIP included, no 'USA'
      } else {
        reFormattedAddress = `${street_number} ${route}, ${city}, ${state} ${zip}`; // ZIP included, no 'USA'
      }
      $app_home_address.val(reFormattedAddress); // apply it manually
  ...
  if (
    ...
    for (i = 0; i < storage.length; i++) {
      if (
        ...
        storage.key(i) != 'app_first_name' && // Already on this page. but need in local storage to pass to downstream pages of the app
        storage.key(i) != 'app_last_name' && // Already on this page. but need in local storage to pass to downstream pages of the app
        storage.key(i) != 'app_home_address' && // Already on this page. but need in local storage to pass to downstream pages of the app
        storage.key(i) != 'app_phone_number' && // Already on this page. but need in local storage to pass to downstream pages of the app
        storage.key(i) != 'app_email_address' && // Already on this page. but need in local storage to pass to downstream pages of the app
...
function onPlaceChanged() {
  ...
  if (place && place.address_components) {
    ...
    if (address_line2) {
      $('#app_address_line2').val(address_line2);
      reFormattedAddress = `${streetNumber} ${route}, ${address_line2}, ${city}, ${state} ${zip}`; // ZIP included, no 'USA'
    } else {
      reFormattedAddress = `${streetNumber} ${route}, ${city}, ${state} ${zip}`; // ZIP included, no 'USA'
    }
    $app_home_address.val(reFormattedAddress); // apply it manually
  ...
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      $app_home_address.on('input', function () {
        storage.setItem('app_home_address', $app_home_address.val());
