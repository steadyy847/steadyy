...
console.log("raw (before trim) = "+raw);
console.log("raw/length (before trim) = "+raw.length);
...
console.log("formatted = "+formatted);
          ...
          // Set value manually (formatted with %), THEN re-mask to ensure the '%' reappears
          $field.val(formatted);           // First set formatted value
          $field.unmask();                 // Unapply any stale mask
          //$field.mask('000%');            // reapply mask so % is always locked in
          applyPercentMask();
          $field.val(formatted);          // Re-set the formatted value so mask applies it correctly
        ...
        $field.on('keydown', function (e) {
          ...
          if (key === 'Backspace') {
            ...
            // Custom logic: prevent deleting the '%' symbol
          ...
          // Always trigger handlePercentageInput after any backspace or delete
