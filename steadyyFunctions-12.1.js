...
function chkErrors(objectAffected, focusState, object, $form, last_focus) {
    ...
    } else if (objectAffected == 'cigarette') {
        $cigarette.css("border-color", "black");
/*
        if (last_focus == "cigarette_date") {chkErrors('cigarette_date', 'focusout', $cigarette_date, $form, last_focus);}
        if (last_focus == "cigarette_amount") {chkErrors('cigarette_amount', 'focusout', $cigarette_amount, $form, last_focus);}
        if (last_focus == "cigar") {chkErrors('cigar', 'focusout', $cigar, $form, last_focus);}
        if (last_focus == "cigar_date") {chkErrors('cigar_date', 'focusout', $cigar_date, $form, last_focus);}
        if (last_focus == "cigar_amount") {chkErrors('cigar_amount', 'focusout', $cigar_amount, $form, last_focus);}
        if (last_focus == "pipe") {chkErrors('pipe', 'focusout', $pipe, $form, last_focus);}
        if (last_focus == "pipe_date") {chkErrors('pipe_date', 'focusout', $pipe_date, $form, last_focus);}
        if (last_focus == "chew") {chkErrors('chew', 'focusout', $chew, $form, last_focus);}
        if (last_focus == "chew_date") {chkErrors('chew_date', 'focusout', $chew_date, $form, last_focus);}
        if (last_focus == "patch") {chkErrors('patch', 'focusout', $patch, $form, last_focus);}
        if (last_focus == "patch_date") {chkErrors('patch_date', 'focusout', $patch_date, $form, last_focus);}
  */
    } else if (objectAffected == 'cigarette_date') {
        $cigarette_date.css("border-color", "black");
/*
        if (last_focus == "cigarette") {chkErrors('cigarette', 'focusout', $cigarette, $form, last_focus);}
        if (last_focus == "cigarette_amount") {chkErrors('cigarette_amount', 'focusout', $cigarette_amount, $form, last_focus);}
        if (last_focus == "cigar") {chkErrors('cigar', 'focusout', $cigar, $form, last_focus);}
        if (last_focus == "cigar_date") {chkErrors('cigar_date', 'focusout', $cigar_date, $form, last_focus);}
        if (last_focus == "cigar_amount") {chkErrors('cigar_amount', 'focusout', $cigar_amount, $form, last_focus);}
...
//        if (state == "IL") {
        ...
        /* Still need approval of OH & FL - apps pending*/
...
function getRedirectURL(currentPage) {
    ...
    if (currentPage < 11) {
				...
				console.log("BACK IN THE steadyFunctions() JS\n"+
										"risk_class="+risk_class+"\n"+
										"tobacco ="+risk_tobacco);
...
//			$document.getElementById("dob").value = dob;
		...
		storage.setItem('coverage_recommendation_uncapped', coverage_rec);

		if (coverage_rec > 5000000) {
			coverage_rec = 5000000
		}
		storage.setItem('coverage_recommendation', coverage_rec);
		storage.setItem('coverage_recommendation_method', coverage_rec_method);
...
console.log("citizenship_risk_class="+citizenship_risk_class+"\n"+
						"driving_risk_class="+driving_risk_class+"\n"+
						"tobacco_risk_class="+tobacco_risk_class+"\n"+
						"bmi_risk_class="+bmi_risk_class+"\n"+
...
var riskClasses = [
    {name: "citizenship_risk_class", value: citizenship_risk_class},
    {name: "driving_risk_class", value: driving_risk_class},
    {name: "tobacco_risk_class", value: tobacco_risk_class},
    {name: "bmi_risk_class", value: bmi_risk_class},
    {name: "cancer_risk_class", value: cancer_risk_class},
    {name: "heart_attack_risk_class", value: heart_attack_risk_class},
    {name: "stroke_risk_class", value: stroke_risk_class},
    {name: "diabetes_risk_class", value: diabetes_risk_class},
    {name: "depression_risk_class", value: depression_risk_class},
    {name: "asthma_risk_class", value: asthma_risk_class},
    {name: "sleep_apnea_risk_class", value: sleep_apnea_risk_class}
];
...
console.log("The final risk class (taking the smallest value) is: " + smallestValue);
	factors_evaluated_for_risk_classes = factors_evaluated_for_risk_classes + "The final risk class (taking the smallest value) is: " + smallestValue;
