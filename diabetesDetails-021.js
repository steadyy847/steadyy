	...
	$(function() {
		...
		(function($, window, document, undefined) {
            ...
            daily_insulin_errors = false;
				...
				if (numericPartOfCommitPoint > 8) {
					$('#commit_point').val(commitPointItem);
				} else {
					storage.setItem('commit_point', "8-diabetes");
					$('#commit_point').val("8-diabetes");
				}
				storage.setItem('navigation_from', "8-diabetes");
...
/* STEP 3 - Evaluate whether there was data in localStorage for the page, and re-initialize it with that data, if there was */
  			...
  			if (storage_diabetes_type !== null) {
  				...
  				$("input[name='DiabetesType'][value='" + storage_diabetes_type + "']").prev('.w-radio-input').addClass('w--redirected-checked');
  				$("#diabetes_type_" + storage_diabetes_type.toLowerCase()).parent().css( "backgroundColor", "#48bd9d" );
  				$("#diabetes_type_" + storage_diabetes_type.toLowerCase()).parent().css( "border", "#48bd9e" );
  				$("#diabetes_type_" + storage_diabetes_type.toLowerCase()).parent().css( "color", "#ffffff" );
  			}
          ...
          document.getElementById("a1c_last_year").value = storage_a1c_last_year;
  			...
  			if (storage_complications !== null) {
  				...
  				$("input[name='Complications'][value='" + storage_complications + "']").prev('.w-radio-input').addClass('w--redirected-checked');
  				$("#complications_" + storage_complications.toLowerCase()).parent().css( "backgroundColor", "#48bd9d" );
  				$("#complications_" + storage_complications.toLowerCase()).parent().css( "border", "#48bd9e" );
  				$("#complications_" + storage_complications.toLowerCase()).parent().css( "color", "#ffffff" );
  			}
...
/* STEP 4 - Set up listeners on each field for input events & validate all date entry fields */
          ...
          if (submit_errors) {
           ...
           && $daily_insulin.length > 0
           		...
           		document.getElementById("diabetes-more-step").disabled = false;
        			$diabetes_more_next.toggleClass(className, true);
        			submit_errors = false;
   					  $(".page_error").hide(500);
					} else {
           		document.getElementById("diabetes-more-step").disabled = true;
        			$diabetes_more_next.toggleClass(className, false);
        			submit_errors = true;
          }
