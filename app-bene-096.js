    ...
    $(form).each(function () {
      ...
      $('.allocation-percentage').each(function () {
        ...
        $field.on('input', function () {
          // Always re-parse the raw digits from the field's actual value
          const inputVal = $field.val();
          const raw = inputVal.replace(/\D/g, '').slice(0, 3);
          const num = parseInt(raw, 10);
          const formatted = num + '%';

          // Hide both errors up front to ensure proper reset
          $errorRequired.hide(500);
          $errorInvalid.hide(500);
          ...
          // Format and apply masked value
          if (inputVal !== formatted) {
            // Avoid triggering recursive input events
            $field.off('input');
            $field.val(formatted);
            setTimeout(() => {
          ...
          // Valid number: clear any lingering errors again
          if (num >= 1 && num <= 100) {
            $errorRequired.hide(500);
            $errorInvalid.hide(500);
          }
