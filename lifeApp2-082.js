...
*/
	        // Retrieve the saved_app parameter that was passed in the URL
//	        const savedApp = getQueryParameter('saved_app');
          ...
          const urlParams = new URLSearchParams(window.location.search);
	        ...
	        // If the saved_app query params in the URL is found and populated, then reset the 
					// submitButton href to take user back to the saved applications page, instead of onto app page 3
	        if (savedApp != '') {
	       		location.href = '/saved-applications/'+savedApp;
	        } else {
	          alert("didn't find query parms in URL, meaning that we didn't get here from the saved-applications page, so proceed to page 3 of the application");
	        }
	      };
			})
		});
