			...
			$(form).each(function() {
			  ...
			  var autocomplete = new google.maps.places.Autocomplete(gpaInput, {
				...
				$home_address.on('input.True', function() {
					if (this.value.length == 0) {
						...
						console.log("In the address input() function. this.value.lenth == 0. Show 'Address Required' error meessage");
						$("#error-address-not-valid").hide(500);
	          ...
	          $home_address.css("border-color", error_color);
				...
				$home_address.on('focusout', function() {
					console.log("In the focusout function\n"+
																									"$home_address.val().length = "+$home_address.val().length+"\n"+
					...
					if ($home_address.val().length == 0) {
						console.log("In the focusout() function. $home_address.val().lenth == 0. Show 'Address Required' error meessage");
						$("#error-address-not-valid").hide(500);
						$("#error-address-required").show(500);
	          $home_address.css("border-color", error_color);
					} else if ($home_address.val().length > 0 && formattedAddress != '' && address_line1 != '' && city != '' && state != '' && postalCode != '')	{
						console.log("In the focusout() function. All addres fields are filled. Hide all errors.");
						$("#error-address-not-valid").hide(500);
						$("#error-address-required").hide(500);
				    $home_address.css("border-color", no_error_color);
					} else if ($home_address.val().length > 0 && formattedAddress != '' && (address_line1 != '' || city != '' || state != '' || postalCode != ''))	{
						// If there was a Google Maps API returned location, but it isn't a complete address (e.g. just city or neighborhood), show message that need users help getting a 'good' formatted address
						console.log("In the focusout() function. Some address fields are filled, but not all, meaning its a validity problem. Show 'Address Not Valid' error message");
						$("#error-address-required").hide(500);
						$("#error-address-not-valid").show(500);
				    $home_address.css("border-color", error_color);
					} else if ($home_address.val().length > 0 && formattedAddress == '') {
						// this seems to trigger after the user input some characters and clicked on a selection from the drop down, but before the call 
						// to the Google Maps API, to physically select an option so it flashes the error message briefly, which will then quickly get replaced 
						// because a valid address was selected. Need a way to avoid doing this under this scenario, and only trigger this when the user manually
						// moved off of the field WITHOUT selecting an option from the drop-down.
						console.log("In the focusout() function. Some address field was started and has some characters, Google Maps API wasn't triggered, meaning its a validity problem. Show 'Address Not Valid' error message");
						$("#error-address-required").hide(500);
						$("#error-address-not-valid").show(500);
				    $home_address.css("border-color", error_color);
					}
...
/*				$first_name.on('focusin', function() {
					if ($home_address.val().length > 0 && formattedAddress == '') {
						console.log("In the focusin() function of $first_name. User started to type in the address field, but the Google Maps API wasn't triggered, meaning we don't yet have a valid formatted address. Show 'Address Not Valid' error message on the Home Address field");
						$("#error-address-required").hide(500);
						$("#error-address-not-valid").show(500);
				    $home_address.css("border-color", error_color);
					}
				...
				$last_name.on('focusin', function() {
					if ($home_address.val().length > 0 && formattedAddress == '') {
						console.log("In the focusin() function of $last_name. User started to type in the address field, but the Google Maps API wasn't triggered, meaning we don't yet have a valid formatted address. Show 'Address Not Valid' error message on the Home Address field");
						$("#error-address-required").hide(500);
						$("#error-address-not-valid").show(500);
				    $home_address.css("border-color", error_color);
					}
				...
				$email_address.on('focusin', function() {
					if ($home_address.val().length > 0 && formattedAddress == '') {
						console.log("In the focusin() function of $email_address. User started to type in the address field, but the Google Maps API wasn't triggered, meaning we don't yet have a valid formatted address. Show 'Address Not Valid' error message on the Home Address field");
						$("#error-address-required").hide(500);
						$("#error-address-not-valid").show(500);
				    $home_address.css("border-color", error_color);
					}
				...
				$phone_number.on('focusin', function() {
					if ($home_address.val().length > 0 && formattedAddress == '') {
						console.log("In the focusin() function of $phone_number. User started to type in the address field, but the Google Maps API wasn't triggered, meaning we don't yet have a valid formatted address. Show 'Address Not Valid' error message on the Home Address field");
						$("#error-address-required").hide(500);
						$("#error-address-not-valid").show(500);
				    $home_address.css("border-color", error_color);
					}
...
*/
				$('form').on('focusin', 'input, select, textarea', function() {
					...
					if (focusedElement.id != "home_address") {
						if ($home_address.val().length > 0 && formattedAddress == '') {
							console.log("In the focusin() function. It appears that the user started to type something into the address field, but navigated off of the field before selecting from the drop-down, meaning the Google Maps API wasn't triggered and we don't yet have a valid formatted address. Show 'Address Not Valid' error message on the Home Address field");
							$("#error-address-required").hide(500);
							$("#error-address-not-valid").show(500);
					    $home_address.css("border-color", error_color);
						}
