	function submitButtonCheck(document, form, page, height_errors, weight_errors, conditions_errors) {
...
//				weight_valid_error = false,
		...
		if (page == "body") {
			if ($height_feet.val() != '' && /^([\w\']{2,2})?$/.test($height_feet.val()) &&
				...
				storage.setItem('height_feet', $height_feet.val());
				storage.setItem('height_inches', $height_inches.val());
				storage.setItem('weight', $weight.val());
console.log("IN BODY PAGE\ntobacco_selected="+tobacco_selected+" | $height_feet.val()="+$height_feet.val()+" | $height_inches.val()="+$height_inches.val()+" | $weight.val()="+$weight.val());
console.log("IN BODY PAGE\ntobacco_selected="+tobacco_selected+" | height_feet="+height_feet+" | height_inches="+height_inches+" | weight="+weight);
//				if (tobacco_selected != undefined && $height_feet.val() !='' && $weight.val()!='') {
//     			if ($height_inches.length == 0) {$height_inches.val() = "0";}
//					var risk = calcRisk(tobacco_selected, $height_feet.val(), $height_inches.val(), $weight.val());
