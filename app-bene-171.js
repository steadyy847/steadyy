...
function validateTotalAllocation() {
  ...
  if ($("input[name='PrimaryBene']:checked").val() == 'No') {
    $('#error-primary-bene-allocation-calc').hide();
  } else if ($("input[name='PrimaryBene']:checked").val() == 'Yes') {
    if (total !== 100) {
  ...
  (function ($, window, document, undefined) {
    ...
    $(form).each(function () {
      ...
      $('.allocation').each(function () {
        ...
        // Apply input mask so users can only enter up to 3 digits followed by a '%' sign.
        // This visually restricts the format but does not enforce validity (e.g., 000–999 still allowed).
        $field.mask('000%');

        function handlePercentageInput() {
          requestAnimationFrame(() => {
            // Strip all non-digit characters (e.g., '%' or accidental characters) to isolate numeric input
            let raw = $field.val().replace(/\D/g, '');

            // Enforce a maximum of 3 digits by truncating anything longer (e.g., 5000 → 500)
            if (raw.length > 3) {
              raw = raw.slice(0, 3);
            }
            ...
            const hasDigits = raw.length > 0;

            if (!hasDigits) {
              // Empty field: clear and reapply mask
              $field.unmask().val('');             // Fully clear input and remove mask temporarily
              $field.mask('000%');                 // Reapply mask to guide re-entry
              $errorRequired.show(500);            // Show "This field is required" error
              $errorInvalid.hide(500);             // Hide "Invalid value" error
              return;
            }

            // If the number is invalid (e.g., 0 or >100), show "Invalid" error
            if (isNaN(num) || num === 0 || num > 100) {
              $errorRequired.hide(500);            // Hide 'required' since something is entered
              $errorInvalid.show(500);             // Show "must be 1–100%" error
            } else {
              $errorInvalid.hide(500);             // Otherwise hide any error
            }

            // Force trailing '%' manually
            const formatted = raw + '%';
            // Force the value back into the field with corrected formatting
            $field.unmask().val(formatted).mask('000%'); // Always show trailing %

            // Run total allocation validator to make sure combined %s across beneficiaries == 100
            validateTotalAllocation();

            // Check if the submit button should be enabled or disabled based on all inputs
            submitButtonCheck();
          });
        }
        ...
        $field.on('keydown', function (e) {
          if (e.key === 'Backspace' || e.key === 'Delete') {
            requestAnimationFrame(() => handlePercentageInput());
          }
        });
        ...
        $field.on('focus', function () {
          if (!$field.val().includes('%')) {
            $field.val($field.val().replace(/\D/g, '') + '%').mask('000%');
          }
        });
        */
