    ...
    $(form).each(function () {
      ...
      autocomplete.addListener('place_changed', function () {
        ...
        if (place && place.address_components) {
          ...
          if (house_number) {
            document.getElementById("app_house_number").val() = house_number;
            storage.setItem('app_house_number', house_number);
          }
          if (street) {
            document.getElementById("app_street").val() = street;
            storage.setItem('app_street', street);
          }
          if (house_number && street) {
            address_line1 = house_number + ' ' + street;
            document.getElementById("app_address_line1").val() = address_line1;
            storage.setItem('app_address_line1', address_line1);
          }
          if (address_line2) {
            document.getElementById("app_address_line2").val() = address_line2;
            storage.setItem('app_address_line2', address_line2);
          }
          if (city) {
            document.getElementById('app_city').val() = city;
            $('#app_city').val(city);
            storage.setItem('app_city', city);
          }
          if (state) {
            document.getElementById('app_state').val() = state;
            $('#app_state').val(state);
            storage.setItem('app_state', state);
          }
          if (postalCode) {
            document.getElementById('app_zip').val() = postalCode;
      ...
      $('form').on('change', 'input, select, textarea', function () {
        if (enabled) {
          submitButtonCheck();
          console.log("in change() event. $app_last_name.val() = "+$app_last_name.val());
        }
      });
      ...
      $('form').on('keyup', 'input, select, textarea', function () {
        if (enabled) {
          submitButtonCheck();
          console.log("in keyup() event. $app_last_name.val() = "+$app_last_name.val());
        }
      });
      ...
      $app_last_name.on('input.True', function () {
        console.log("In the $app_last_name.on(input.True) function.\n"+
                    "this.val() = "+this.val()+"\n"+
      ...
      $app_last_name.on('focusout', function () {
        console.log("In the $app_last_name.on(focusout) function.\n"+
                    "this.val() = "+this.val()+"\n"+
                    "this.val().length = "+this.val().length);
      ...
      function submitButtonCheck() {
        ...
        console.log("In the submitButtonCheck function. ALL CHECKS PASSED. ENABLING SUBMIT BUTTON\n"+
            ...
            if (status === 'OK' && results[0]) {
              ...
              for (var i = 0; i < address_components.length; i++) {
                ...
                if (component.types.includes('street_number')) {
                  ...
                  document.getElementById('app_house_number').val() =
                    house_number;
                  storage.setItem('app_house_number', house_number);
                }
                ...
                if (component.types.includes('subpremise')) {
                  ...
                  document.getElementById('app_address_line2').val() =
                    address_line2;
                  storage.setItem('app_address_line2', address_line2);
                }
              ...
              if (house_number && street) {
                address_line1 = house_number + ' ' + street;
                document.getElementById('app_address_line1').val() =
                  address_line1;
                storage.setItem('app_address_line1', address_line1);
                submitButtonCheck();
              }
