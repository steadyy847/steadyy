    ...
    $(form).each(function () {
      ...
      $('.allocation').each(function () {
        ...
        function handlePercentageInput() {
          ...
          // Create a formatted string like '75%' if it's a valid number; otherwise blank
          const formatted = isNaN(num) ? '' : num + '%';

          // If the field is now empty, clear the mask and show 'required' error
          if (raw === '') {
          ...
          $field.val(formatted);
          $field.unmask().mask('000%');  // <-- this re-applies the suffix properly after .val()
