...
*/    
	...
	if (storage.getItem('state') == "NY") {
		compinc = "PRUJ,JOHY,PROA"; // The Brokerage offers drop ticket for Prudential in NY State, but Protective & John Hancock have to be submitted as traditional PDF filled apps
	} else {
		compinc = "AMGE,CINN,BANN,PACL,PROT,PRUC,PRUJ,SBLI,JOHU,LNNA";
	}
  ...
  var formData = {
	...
	$.ajax(settings).done(function (response) {
		...
		if (response.Compulife_ComparisonResults === undefined) {
			...
			var years_to_cover;
			years_to_cover = 65-Number(storage.getItem('age'));
			console.log("years_to_cover = " + years_to_cover);
			console.log("recommended term (based on nearest 5 up) = " + (Math.ceil(years_to_cover / 5) * 5));
			console.log("recommended term (based on nearest 5) = " + (Math.round(years_to_cover / 5) * 5));

//			$('#recommendation_10').show(500);
